...
```
## ✓ RIGHT:  'If H₀ is true, we'd see data this extreme only 3% of the time'
```
...
```
## ❌ WRONG: 'p = 0.03 means there's a 97% chance the effect is real'
```
...
```
## ✓ RIGHT:  'The p-value says nothing about the probability of H₁'
```
...
```
## ❌ WRONG: 'Not significant means no effect exists'
```
...
```
## ✓ RIGHT:  'Not significant means insufficient evidence to reject H₀'
```
...
```
## ❌ WRONG: 'p = 0.049 is significant, p = 0.051 is not, so they're different'
```
...
```
## ✓ RIGHT:  'These p-values represent nearly identical evidence'
```
...
<div class="figure">
<img src="/courses/statistics-1-foundations/pvalue_distribution-1.png" alt="Distribution of p-values under H₀ and H₁" width="100%" />
<p class="caption">Distribution of p-values under H₀ and H₁</p>
</div>
...
```
## Sample Size Calculations (80% Power, α = 0.05, Two-sided)
```
