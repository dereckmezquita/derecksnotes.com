...
breast_cancer <- data.table$fread("data/bioinformatics/breast_cancer_wisconsin.csv")
...
```{r scatter_basic, fig.cap="Scatter plot revealing positive association between bill length and depth in penguins"}
# Clean penguin data
...
```{r scatter_overplotting, fig.cap="Techniques for handling overplotting in large datasets"}
...
```{r scatter_scratch, fig.cap="Scatter plot as coordinate mapping: each row becomes (x, y) position"}
...
ggplot2$ggplot(small_sample) +
    ...
    ggplot2$geom_text(
        ggplot2$aes(x = bill_length_mm, y = bill_depth_mm, label = seq_len(.N)),
        vjust = -1,
        size = 3
    ) +
    ggplot2$labs(
...
```{r trend_lines, fig.cap="Linear regression assumes linearity; LOESS reveals the true relationship shape"}
...
```{r lm_from_scratch, fig.cap="Linear regression line computed from scratch"}
...
```{r scatter_matrix, fig.cap="Scatter plot matrix showing all pairwise relationships among penguin measurements"}
...
```{r grouped_boxplot, fig.cap="Grouped box plots comparing BMI distributions across education levels"}
...
```{r grouped_violin, fig.cap="Violin plots reveal that Gentoo penguins have bimodal body mass (likely sex dimorphism)"}
...
```{r facet_wrap, fig.cap="Facet wrap showing life expectancy trends by continent"}
...
```{r facet_grid, fig.cap="Facet grid crossing species (rows) with island (columns)"}
...
```{r colour_encoding, fig.cap="Colour encoding reveals Simpson's Paradox: negative overall trend but positive within-species trends"}
...
```{r colourblind, fig.cap="Comparison of default palette with colour-blind friendly alternatives"}
...
```{r correlation_scratch, fig.cap="Correlation matrix computed from scratch and displayed as heatmap"}
...
```{r heatmap, fig.cap="Correlation heatmap with diverging colour scale centred at zero"}
# Convert to long format for ggplot
cor_dt <- as.data.table(cor_matrix, keep.rownames = "var1")
cor_long <- data.table$melt(cor_dt, id.vars = "var1", variable.name = "var2", value.name = "correlation")
...
```{r heatmap_large, fig.cap="Correlation heatmap for breast cancer diagnostic features"}
...
bc_cor_long <- data.table$melt(bc_cor_dt, id.vars = "var1", variable.name = "var2", value.name = "correlation")
...
```{r anscombe, fig.cap="Anscombe's quartet: four datasets with identical correlation but very different relationships"}
# Anscombe's quartet demonstrates correlation limitations
# All four datasets have correlation r ≈ 0.816

anscombe_data <- data.table$data.table(
...
anscombe_long <- data.table$rbindlist(list(
    data.table$data.table(x = anscombe_data$x1, y = anscombe_data$y1, dataset = "Dataset 1"),
    data.table$data.table(x = anscombe_data$x2, y = anscombe_data$y2, dataset = "Dataset 2"),
    data.table$data.table(x = anscombe_data$x3, y = anscombe_data$y3, dataset = "Dataset 3"),
    data.table$data.table(x = anscombe_data$x4, y = anscombe_data$y4, dataset = "Dataset 4")
))
