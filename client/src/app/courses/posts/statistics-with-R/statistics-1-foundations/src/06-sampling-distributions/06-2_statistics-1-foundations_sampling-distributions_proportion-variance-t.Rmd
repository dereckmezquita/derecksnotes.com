...
```{r proportion_as_mean, fig.cap="The sample proportion is the mean of binary outcomes"}
set.seed(42)
...
```{r se_proportion, fig.cap="Standard error of proportion depends on both p and n"}
...
```{r normal_approx_binomial, fig.cap="Normal approximation improves as np and n(1-p) increase"}
set.seed(123)
...
```{r variance_distribution, fig.cap="Sample variance follows a scaled chi-square distribution"}
set.seed(456)
...
```{r bessel_demonstration, fig.cap="Demonstration that n-1 correction makes variance unbiased"}
set.seed(789)
...
```{r difference_means, fig.cap="Sampling distribution of the difference between two means"}
set.seed(101)
...
```{r difference_proportions, fig.cap="Sampling distribution of the difference between two proportions"}
set.seed(202)
...
```{r t_distribution_intro, fig.cap="The t-distribution has heavier tails than the normal, especially for small samples"}
...
```{r t_convergence, fig.cap="The t-distribution converges to normal as sample size increases"}
set.seed(303)
...
```{r t_application, fig.cap="Practical application: blood pressure study with n=25"}
set.seed(404)
...
```{r nhanes_application, fig.cap="Sampling distributions from NHANES demonstrate real-world variability"}
set.seed(505)
