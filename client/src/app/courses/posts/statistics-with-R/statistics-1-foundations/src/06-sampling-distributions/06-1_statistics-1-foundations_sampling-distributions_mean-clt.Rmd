...
knitr::opts_chunk$set(dpi = 300, fig.width = 10, fig.height = 7, collapse = FALSE, results = 'hold')
```
...
```{r sampling_variability, fig.cap="Each sample from the same population produces a different sample mean"}
set.seed(42)
...
```{r simulate_sampling_dist, fig.cap="Simulating the sampling distribution by repeated sampling"}
set.seed(123)
...
```{r variance_demo, fig.cap="Variance of the sample mean decreases with sample size"}
...
```{r se_vs_sd, fig.cap="Standard deviation describes individual variation; standard error describes variation in sample means"}
set.seed(456)
...
```{r sample_size_effect, fig.cap="The sqrt(n) relationship: quadrupling sample size halves the standard error"}
set.seed(789)
...
```{r clt_remarkable, fig.cap="The CLT works for any population distribution"}
set.seed(101)
...
```{r clt_demo, fig.cap="The CLT transforms any distribution into a normal distribution of sample means"}
set.seed(202)
...
```{r sample_size_requirement, fig.cap="More skewed populations require larger samples for CLT to apply"}
set.seed(303)
...
```{r convolution_demo, fig.cap="Adding random variables progressively smooths the distribution"}
set.seed(404)
...
```{r nhanes_sampling_dist, fig.cap="Sampling distribution of mean BMI from NHANES data"}
set.seed(505)
...
```{r power_demo, fig.cap="How sample size affects the precision of clinical trial estimates"}
