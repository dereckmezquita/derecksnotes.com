...
```{r conditional_definition, fig.cap="Conditional probability restricts the sample space"}
...
```{r multiplication_rule, fig.cap="The multiplication rule finds joint probabilities from conditionals"}
...
```{r tree_diagram_detailed, fig.cap="Tree diagram for a two-stage medical screening process"}
...
```{r sens_spec_demo, fig.cap="Sensitivity and specificity measure different aspects of test performance"}
...
```{r independence_definition, fig.cap="Testing for independence in NHANES data"}
...
```{r mutually_exclusive_vs_independent, fig.cap="Mutually exclusive events are maximally dependent, not independent"}
...
```{r independence_medical, fig.cap="Risk factors often cluster, violating independence"}
...
```{r bayes_derivation, fig.cap="Bayes' theorem connects forward and inverse probabilities"}
...
```{r bayes_components, fig.cap="The components of Bayes' theorem in belief updating"}
...
```{r base_rate_fallacy, fig.cap="The base rate fallacy: ignoring prior probabilities"}
...
```{r diagnostic_metrics, fig.cap="The four diagnostic test metrics"}
...
```{r rare_disease_screening, fig.cap="The false positive problem in rare disease screening"}
...
```{r law_total_probability, fig.cap="The law of total probability partitions the sample space"}
...
```{r visual_aids, fig.cap="Icon arrays help patients understand test results"}
