...
```{r multiplication_principle, fig.cap="The multiplication principle counts sequential choices"}
...
```{r permutations, fig.cap="Permutations count ordered arrangements"}
...
```{r combinations, fig.cap="Combinations count unordered selections"}
...
```{r pascals_triangle, fig.cap="Pascal's triangle shows the structure of binomial coefficients"}
...
```{r counting_probability, fig.cap="Using combinatorics to calculate probabilities"}
...
```{r monte_carlo_intro, fig.cap="Monte Carlo estimation of probability"}
...
```{r birthday_problem, fig.cap="The birthday problem: probability of shared birthdays"}
...
```{r monty_hall, fig.cap="The Monty Hall problem: always switch!"}
...
```{r clinical_simulation, fig.cap="Simulating a clinical trial to estimate power"}
...
```{r law_large_numbers, fig.cap="The law of large numbers: sample means converge to population mean"}
...
```{r setting_seeds, fig.cap="Seeds ensure reproducible random sequences"}
...
```{r when_simulation, fig.cap="Comparing exact and simulated probabilities"}
