...
covid <- data.table$fread("data/medical/covid_testing.csv")
lung <- data.table$fread("data/supplementary/lung.csv")

cat("Datasets loaded:\n")
cat("  NHANES:", nrow(nhanes), "observations\n")
cat("  Gapminder:", nrow(gapminder), "observations\n")
cat("  COVID testing:", nrow(covid), "observations\n")
...
```{r qq_basic, fig.cap="QQ plot comparing BMI to normal distribution"}
# BMI data
bmi <- nhanes[!is.na(BMI), BMI]

# Basic QQ plot
ggplot2$ggplot(data.table$data.table(bmi = bmi), ggplot2$aes(sample = bmi)) +
...
```{r qq_scratch, fig.cap="QQ plot constructed manually showing the matching of sample to theoretical quantiles"}
...
set.seed(42)
age_sample <- nhanes[!is.na(Age), Age][sample(.N, 200)]
...
```{r qq_patterns, fig.cap="QQ plot patterns for different distribution shapes"}
set.seed(123)
n <- 500
...
patterns <- data.table$rbindlist(list(
    data.table$data.table(value = normal_data, distribution = "Normal"),
    data.table$data.table(value = heavy_tails, distribution = "Heavy Tails (t, df=3)"),
    data.table$data.table(value = right_skew, distribution = "Right Skew (Exponential)"),
    data.table$data.table(value = left_skew, distribution = "Left Skew")
))
...
```{r qq_residuals, fig.cap="QQ plot of regression residuals to check normality assumption"}
# Fit a simple linear model
bmi_bp_model <- lm(BPSysAve ~ BMI + Age, data = nhanes[!is.na(BMI) & !is.na(BPSysAve) & !is.na(Age)])

# Extract residuals
residuals_dt <- data.table$data.table(residual = residuals(bmi_bp_model))
...
```{r lineplot_basic, fig.cap="Line plot showing life expectancy trends over time by continent"}
...
```{r lineplot_covid, fig.cap="Daily COVID-19 testing volume over pandemic days"}
# Aggregate COVID tests by day
covid_daily <- covid[, .(
    n_tests = .N,
    n_positive = sum(result == "positive", na.rm = TRUE)
), by = pan_day]

covid_daily[, positivity_rate := n_positive / n_tests]

# Dual-panel plot
p1 <- ggplot2$ggplot(covid_daily, ggplot2$aes(x = pan_day, y = n_tests)) +
...
p2 <- ggplot2$ggplot(covid_daily, ggplot2$aes(x = pan_day, y = positivity_rate)) +
...
```{r spaghetti, fig.cap="Spaghetti plot showing individual country trajectories within each continent"}
...
```{r spaghetti_highlight, fig.cap="Highlighting specific country trajectories against the background of all countries"}
...
```{r ribbon_plot, fig.cap="Mean trajectory with 95% confidence band showing uncertainty over time"}
...
```{r percentile_band, fig.cap="Median trajectory with interquartile range showing individual variation"}
...
```{r multi_panel, fig.cap="Multi-panel figure combining different plot types for comprehensive data summary"}
...
```{r annotation, fig.cap="Annotated figure showing key features for publication"}
...
```{r stat_annotation, fig.cap="Box plot with statistical comparison annotations"}
# Compare groups with significance indicators
species_mass <- penguins <- data.table$fread("data/primary/penguins.csv")
