...
```{r point_vs_interval, fig.cap="Point estimates vs interval estimates"}
set.seed(123)
...
```{r coverage_demo, fig.cap="The frequentist interpretation of confidence intervals"}
set.seed(456)
...
```{r confidence_levels, fig.cap="Effect of confidence level on interval width"}
set.seed(789)
...
```{r derivation_visual, fig.cap="Visual derivation of confidence interval from the sampling distribution"}
...
```{r ci_components, fig.cap="Components of a confidence interval"}
...
```{r sample_size_effect, fig.cap="Effect of sample size on confidence interval width"}
set.seed(123)
true_mu <- 100
true_sd <- 20
...
```{r variability_effect, fig.cap="Effect of population variability on confidence interval width"}
set.seed(456)
n <- 50
true_mu <- 100
...
```{r misinterpretation_demo, fig.cap="Confidence interval vs data range"}
set.seed(123)
