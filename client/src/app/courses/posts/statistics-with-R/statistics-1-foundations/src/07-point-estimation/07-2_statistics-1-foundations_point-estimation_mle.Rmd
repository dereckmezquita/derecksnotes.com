...
```{r likelihood_concept, fig.cap="The likelihood function: same formula as probability, but viewed differently"}
...
```{r log_likelihood, fig.cap="Log-likelihood is easier to work with computationally"}
...
```{r mle_normal, fig.cap="MLE for normal distribution parameters"}
set.seed(123)
...
```{r mle_asymptotic, fig.cap="MLEs are consistent and asymptotically normal"}
set.seed(789)
...
```{r fisher_info, fig.cap="Standard error of MLE comes from Fisher information"}
set.seed(202)
...
```{r numerical_mle, fig.cap="Numerical optimisation to find MLE for beta distribution"}
set.seed(303)
...
```{r mle_gamma_scratch, fig.cap="MLE for gamma distribution using numerical optimisation"}
set.seed(404)
...
```{r survival_application, fig.cap="Estimating disease progression rate using MLE"}
