...
#>   λ = 0.5: E(X) = 2.00, SD(X) = 2.00
#>   λ = 1.0: E(X) = 1.00, SD(X) = 1.00
#>   λ = 2.0: E(X) = 0.50, SD(X) = 0.50
#>   λ = 5.0: E(X) = 0.20, SD(X) = 0.20
...
cat(sprintf("  Mean time between arrivals: 1/λ = %.2f hours = %.0f minutes\n\n",
            1/lambda, 60/lambda))
#>   Mean time between arrivals: 1/λ = 0.33 hours = 20 minutes
...
#> Failure rate λ = 1/MTTF = 0.0010 per hour
...
#> 
#> Hazard rate (failure rate): λ = 0.0010 per hour (constant)
cat("The exponential assumes constant hazard — no wear-out or improvement.\n")
#> The exponential assumes constant hazard — no wear-out or improvement.
```
...
#>   α (shape): controls shape of distribution
cat("  β (rate): controls scale (higher β = more compressed)\n\n")
#>   β (rate): controls scale (higher β = more compressed)
...
#>   α=1, β=1 (Exponential): E(X) = 1.00, Var(X) = 1.00
#>   α=2, β=1: E(X) = 2.00, Var(X) = 2.00
#>   α=5, β=1: E(X) = 5.00, Var(X) = 5.00
#>   α=5, β=2: E(X) = 2.50, Var(X) = 1.25
#>   α=0.5, β=1: E(X) = 0.50, Var(X) = 0.50
...
#> 3. ERLANG: Gamma(k, β) with integer k
cat("   Sum of k independent Exp(β) variables\n")
#>    Sum of k independent Exp(β) variables
...
#> Setting: Events at rate λ = 2 per hour
...
#> Support: (0, 1) — perfect for probabilities and proportions
...
#>   α=1, β=1 (Uniform): mean = 0.50
#>   α=2, β=2 (Symmetric): mean = 0.50
#>   α=5, β=2 (Left-skewed): mean = 0.71
#>   α=2, β=5 (Right-skewed): mean = 0.29
#>   α=0.5, β=0.5 (U-shaped): mean = 0.50
#>   α=5, β=1 (J-shaped): mean = 0.83
...
#>   ν =  2: E(X) =  2, Var(X) =  4
#>   ν =  5: E(X) =  5, Var(X) = 10
#>   ν = 10: E(X) = 10, Var(X) = 20
#>   ν = 20: E(X) = 20, Var(X) = 40

cat("\nNote: E(X) = ν and Var(X) = 2ν\n")
#> 
#> Note: E(X) = ν and Var(X) = 2ν
cat("As ν → ∞, χ² approaches normal distribution\n")
#> As ν → ∞, χ² approaches normal distribution
...
#> Verification: χ²(5) = Sum of 5 Squared Standard Normals
cat("=======================================================\n\n")
#> =======================================================

cat("Direct χ²(5) generation:\n")
#> Direct χ²(5) generation:
...
#>   • Same symmetric bell shape
cat("  • Heavier tails (more extreme values likely)\n")
#>   • Heavier tails (more extreme values likely)
...
#> When we don't know σ (population SD), we estimate it with s (sample SD).
cat("This introduces additional uncertainty beyond the sampling of X̄.\n\n")
#> This introduces additional uncertainty beyond the sampling of X̄.
...
#>   • With small samples, uncertainty in s is large → heavier tails
cat("  • With large samples, s → σ → t approaches normal\n\n")
#>   • With large samples, s → σ → t approaches normal
...
#>   • n ≥ 30: t ≈ normal (but using t is still correct)
cat("  • n → ∞: t → normal\n")
#>   • n → ∞: t → normal
```
...
#>   ├─ Count events at constant rate? → Poisson
...
#>   └─ Categorical frequencies? → Chi-square
```
...
#>    - Waiting times are never negative and often have a long tail → Exponential
cat("   - Proportions are between 0 and 1 → Beta\n")
#>    - Proportions are between 0 and 1 → Beta
cat("   - Measurements like height cluster around an average → Normal'\n\n")
#>    - Measurements like height cluster around an average → Normal'
...
#>    - The average is [Z] with uncertainty ±[W]
