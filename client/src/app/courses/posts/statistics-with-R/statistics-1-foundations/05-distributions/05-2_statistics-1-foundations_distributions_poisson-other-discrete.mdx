...
#> Convergence of Binomial to Poisson (λ = 5)
cat("==========================================\n\n")
#> ==========================================
...
#> Average admissions per hour: λ = 2.1
...
#>   Expected mutations per gene: λ = 0.0100
...
#> Across 100 tumour samples (λ = 0.3 per sample per gene):
...
#>   Ratio > 1: Overdispersion — consider negative binomial
cat("  Ratio < 1: Underdispersion — rare, may indicate data issues\n")
#>   Ratio < 1: Underdispersion — rare, may indicate data issues
```
...
#>    → (Especially if sample > 5% of population)
...
#>    → Use BINOMIAL (for fixed n)
cat("   → Use POISSON (for rare events, large n)\n")
#>    → Use POISSON (for rare events, large n)
```
