...
covid_testing <- fread("../data/medical/covid_testing.csv")

cat("Datasets loaded:\n")
#> Datasets loaded:
cat("  NHANES:", nrow(nhanes), "observations\n")
#>   NHANES: 10000 observations
cat("  COVID Testing:", nrow(covid_testing), "observations\n")
#>   COVID Testing: 15524 observations
```
...
<Figure src="/courses/statistics-1-foundations/poisson_intro-1.png" alt="The Poisson distribution for different rate parameters">
	The Poisson distribution for different rate parameters
</Figure>
...
<Figure src="/courses/statistics-1-foundations/poisson_binomial_limit-1.png" alt="Poisson as the limit of binomial for rare events">
	Poisson as the limit of binomial for rare events
</Figure>
...
#> Convergence of Binomial to Poisson (<U+03BB> = 5)
cat("==========================================\n\n")
#> ==========================================
...
#>   Binomial(10, 0.5000): 0.246094
#>   Binomial(50, 0.1000): 0.184925
#>   Binomial(100, 0.0500): 0.180018
#>   Binomial(500, 0.0100): 0.176351
#>   Binomial(1000, 0.0050): 0.175908
...
#> Poisson PMF Components: <U+03BB> = 4
cat("==============================\n\n")
#> ==============================

cat("P(X = k) = (λ^k × e^(-λ)) / k!\n\n")
#> P(X = k) = (<U+03BB>^k <U+00D7> e^(-<U+03BB>)) / k!
...
print(pmf_dt[, .(k,
                 ...
                 `k!` = k_factorial,
                 `P(X=k)` = round(P_X_k, 6))])
#>         k <U+03BB>^k e^(-<U+03BB>)        k!   P(X=k)
#>     <int>      <num>         <num>     <num>    <num>
#>  1:     0          1      0.018316         1 0.018316
#>  2:     1          4      0.018316         1 0.073263
#>  3:     2         16      0.018316         2 0.146525
#>  4:     3         64      0.018316         6 0.195367
#>  5:     4        256      0.018316        24 0.195367
#>  6:     5       1024      0.018316       120 0.156293
#>  7:     6       4096      0.018316       720 0.104196
#>  8:     7      16384      0.018316      5040 0.059540
#>  9:     8      65536      0.018316     40320 0.029770
#> 10:     9     262144      0.018316    362880 0.013231
#> 11:    10    1048576      0.018316   3628800 0.005292
#> 12:    11    4194304      0.018316  39916800 0.001925
#> 13:    12   16777216      0.018316 479001600 0.000642

cat("\nSum of probabilities:", round(sum(pmf_dt$P_X_k), 6), "\n")
#> 
#> Sum of probabilities: 0.999726
...
<Figure src="/courses/statistics-1-foundations/poisson_pmf_components-1.png" alt="Components of the Poisson PMF">
	Components of the Poisson PMF
</Figure>
...
#>    lambda  E(X) Var(X) Sample Mean Sample Var
#>     <num> <num>  <num>       <num>      <num>
#> 1:      1     1      1       1.005      1.006
#> 2:      2     2      2       1.995      2.004
#> 3:      5     5      5       4.994      4.968
#> 4:     10    10     10      10.004     10.026
#> 5:     20    20     20      19.983     19.818
...
<Figure src="/courses/statistics-1-foundations/poisson_mean_var-1.png" alt="Poisson distributions always have mean = variance">
	Poisson distributions always have mean = variance
</Figure>
...
#> Poisson Functions: From Scratch vs R Built-in
cat("==============================================\n\n")
#> ==============================================

cat(sprintf("Parameter: λ = %.1f\n\n", lambda))
#> Parameter: <U+03BB> = 5.0
...
#>   P(X = 0): ours = 0.00673795, R = 0.00673795
#>   P(X = 2): ours = 0.08422434, R = 0.08422434
#>   P(X = 5): ours = 0.17546737, R = 0.17546737
#>   P(X = 8): ours = 0.06527804, R = 0.06527804
#>   P(X = 10): ours = 0.01813279, R = 0.01813279
#>   P(X = 15): ours = 0.00015725, R = 0.00015725
...
#>   P(X <= 0): ours = 0.006738, R = 0.006738
#>   P(X <= 2): ours = 0.124652, R = 0.124652
#>   P(X <= 5): ours = 0.615961, R = 0.615961
#>   P(X <= 8): ours = 0.931906, R = 0.931906
#>   P(X <= 10): ours = 0.986305, R = 0.986305
...
#> Hospital A&E Admissions Example
cat("===============================\n\n")
#> ===============================

cat(sprintf("Average admissions per hour: λ = %.1f\n\n", lambda_hourly))
#> Average admissions per hour: <U+03BB> = 2.1
...
<Figure src="/courses/statistics-1-foundations/poisson_hospital-1.png" alt="Poisson model for hospital admissions">
	Poisson model for hospital admissions
</Figure>
...
#> Somatic Mutation Counting Example
cat("=================================\n\n")
#> =================================
...
#>   Background mutation rate: 1 per Mb
cat("  Gene size: 10 kb\n")
#>   Gene size: 10 kb
cat(sprintf("  Expected mutations per gene: λ = %.4f\n\n", lambda_mutations))
#>   Expected mutations per gene: <U+03BB> = 0.0100
...
<Figure src="/courses/statistics-1-foundations/poisson_mutations-1.png" alt="Poisson model for mutation counts">
	Poisson model for mutation counts
</Figure>
...
#> Checking Poisson Assumptions: Mean vs Variance
cat("===============================================\n\n")
#> ===============================================
...
#>   Mean: 4.254, Variance: 12.515, Ratio: 2.942

cat("\nInterpretation:\n")
#> 
#> Interpretation:
cat("  Ratio ≈ 1: Poisson is appropriate\n")
#>   Ratio <U+2248> 1: Poisson is appropriate
cat("  Ratio > 1: Overdispersion — consider negative binomial\n")
#>   Ratio > 1: Overdispersion <U+2014> consider negative binomial
cat("  Ratio < 1: Underdispersion — rare, may indicate data issues\n")
#>   Ratio < 1: Underdispersion <U+2014> rare, may indicate data issues
```
...
#> p = 0.1: E(X) = 10.00, Var(X) = 90.00
#> p = 0.3: E(X) = 3.33, Var(X) = 7.78
#> p = 0.5: E(X) = 2.00, Var(X) = 2.00
#> p = 0.8: E(X) = 1.25, Var(X) = 0.31
...
<Figure src="/courses/statistics-1-foundations/geometric_dist-1.png" alt="Geometric distribution: trials until first success">
...
#> P(X > t) = P(X > 3) = 0.343000

cat("These are equal! The past doesn't affect the future.\n")
#> These are equal! The past doesn't affect the future.
...
#> r = 1: E(trials) = 3.3, Var = 7.8
#> r = 3: E(trials) = 10.0, Var = 23.3
#> r = 5: E(trials) = 16.7, Var = 38.9
#> r = 10: E(trials) = 33.3, Var = 77.8
...
<Figure src="/courses/statistics-1-foundations/negbinom_dist-1.png" alt="Negative binomial distribution: trials until r successes">
	Negative binomial distribution: trials until r successes
</Figure>
...
#> Poisson vs Negative Binomial for Mean = 5
cat("==========================================\n\n")
#> ==========================================
...
#> NegBin (size=1)       5.0     30.0
#> NegBin (size=2)       5.0     17.5
#> NegBin (size=5)       5.0     10.0
...
<Figure src="/courses/statistics-1-foundations/negbinom_overdispersion-1.png" alt="Negative binomial vs Poisson for overdispersed data">
	Negative binomial vs Poisson for overdispersed data
</Figure>
...
#> Population: N = 100, K = 30 successes
...
<Figure src="/courses/statistics-1-foundations/hypergeometric-1.png" alt="Hypergeometric vs Binomial: sampling without replacement">
	Hypergeometric vs Binomial: sampling without replacement
</Figure>
...
<Figure src="/courses/statistics-1-foundations/hyper_vs_binom_convergence-1.png" alt="Hypergeometric approaches binomial as population grows">
	Hypergeometric approaches binomial as population grows
</Figure>
...
#> Choosing the Right Count Distribution
cat("=====================================\n\n")
#> =====================================
...
#> 1. Are events happening over time/space at a constant rate?
cat("   → Use POISSON\n\n")
#>    <U+2192> Use POISSON

cat("2. Are you counting trials until a fixed number of successes?\n")
#> 2. Are you counting trials until a fixed number of successes?
cat("   → First success: GEOMETRIC\n")
#>    <U+2192> First success: GEOMETRIC
cat("   → r successes: NEGATIVE BINOMIAL\n\n")
#>    <U+2192> r successes: NEGATIVE BINOMIAL

cat("3. Is variance much larger than mean (overdispersion)?\n")
#> 3. Is variance much larger than mean (overdispersion)?
cat("   → Use NEGATIVE BINOMIAL instead of Poisson\n\n")
#>    <U+2192> Use NEGATIVE BINOMIAL instead of Poisson

cat("4. Are you sampling without replacement from a finite population?\n")
#> 4. Are you sampling without replacement from a finite population?
cat("   → Use HYPERGEOMETRIC\n")
#>    <U+2192> Use HYPERGEOMETRIC
cat("   → (Especially if sample > 5% of population)\n\n")
#>    <U+2192> (Especially if sample > 5% of population)

cat("5. Is the population effectively infinite (or sampling with replacement)?\n")
#> 5. Is the population effectively infinite (or sampling with replacement)?
cat("   → Use BINOMIAL (for fixed n)\n")
#>    <U+2192> Use BINOMIAL (for fixed n)
cat("   → Use POISSON (for rare events, large n)\n")
#>    <U+2192> Use POISSON (for rare events, large n)
```
