...
#>   P(High BP <U+2229> High Glucose) = P(High Glucose | High BP) <U+00D7> P(High BP)
...
#>   P(Normal BP <U+2229> High Glucose) = P(High Glucose | Normal BP) <U+00D7> P(Normal BP)
...
#>   Disease <U+2192> Symptoms <U+2192> Test+: 0.01615
cat("  Disease → Symptoms → Test-:", round(p2, 5), "\n")
#>   Disease <U+2192> Symptoms <U+2192> Test-: 0.00085
cat("  Disease → No Symptoms:", round(p3, 5), "\n")
#>   Disease <U+2192> No Symptoms: 0.003
cat("  No Disease → Symptoms → Test+:", round(p4, 5), "\n")
#>   No Disease <U+2192> Symptoms <U+2192> Test+: 0.0098
cat("  No Disease → Symptoms → Test-:", round(p5, 5), "\n")
#>   No Disease <U+2192> Symptoms <U+2192> Test-: 0.1862
cat("  No Disease → No Symptoms:", round(p6, 5), "\n")
#>   No Disease <U+2192> No Symptoms: 0.784
...
#> Screening 10,000 People: The 2<U+00D7>2 Table
cat("======================================\n\n")
#> ======================================
...
#> If independent: P(Female <U+2229> Diabetic) = P(Female) <U+00D7> P(Diabetic)
cat("  Expected:", round(P_female, 4), "×", round(P_diabetic, 4), "=",
    round(expected_joint, 4), "\n")
#>   Expected: 0.502 <U+00D7> 0.076 = 0.0382
...
#> Conclusion: Cannot reject independence at <U+03B1> = 0.05
```
...
#>   P(Type A <U+2229> Type B) = 0 (impossible to have both)
cat("  P(Type A) × P(Type B) =", P_type_A * P_type_B, "\n")
#>   P(Type A) <U+00D7> P(Type B) = 0.042
cat("  Since 0 ≠", P_type_A * P_type_B, ", they are NOT independent.\n\n")
#>   Since 0 <U+2260> 0.042 , they are NOT independent.
...
#>   Since 0 <U+2260> 0.1 , knowing Type A changes P(Type B).
...
#>   P(H2 | H1) = P(H1 <U+2229> H2) / P(H1) = 0.25 / 0.5 = 0.5
cat("  P(H2) =", P_H2, "\n")
#>   P(H2) = 0.5
...
#>   P(+) = P(+ | D) <U+00D7> P(D) + P(+ | ~D) <U+00D7> P(~D)
cat("       =", P_pos_given_D, "×", P_D, "+", P_pos_given_notD, "×", 1-P_D, "\n")
#>        = 0.95 <U+00D7> 0.01 + 0.1 <U+00D7> 0.99
cat("       =", round(P_pos, 4), "\n\n")
#>        = 0.1085
...
cat("  P(D | +) = P(+ | D) × P(D) / P(+)\n")
#>   P(D | +) = P(+ | D) <U+00D7> P(D) / P(+)
cat("           =", P_pos_given_D, "×", P_D, "/", round(P_pos, 4), "\n")
#>            = 0.95 <U+00D7> 0.01 / 0.1085
...
#> Na<U+00EF>ve estimate: 'If I test positive, there's a 99% chance I have the disease'
...
#> P(+) = <U+03A3> P(+ | group) <U+00D7> P(group)
cat("     =", paste(paste0(round(P_pos_given_group, 4), " × ", group_sizes),
                   collapse = " + "), "\n")
#>      = 0.067 <U+00D7> 0.35 + 0.118 <U+00D7> 0.4 + 0.22 <U+00D7> 0.25
...
#> PPV calculated: 0.0876 <U+2248> 9 %
```
