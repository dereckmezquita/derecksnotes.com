...
```
## Estimator: X<U+0304> = (1/n) <U+03A3> X<U+1D62>
```
...
```
## 1000 estimates from samples of n = 25:
```
...
```
## True variance: <U+03C3><U+00B2> = 100
```
...
```
## Key: <n>
##        n Biased (<U+00F7>n) Unbiased (<U+00F7>(n-1))
##    <num>              <num>                    <num>
## 1:     5             -19.71                     0.37
## 2:    10              -9.35                    -0.09
## 3:    25              -3.95                    -0.03
...
```
## 
## Theoretical bias of <U+00F7>n estimator: -<U+03C3><U+00B2>/n
```
...
```
##   n =   5: theoretical = -20.00
##   n =  10: theoretical = -10.00
##   n =  25: theoretical = -4.00
##   n =  50: theoretical = -2.00
##   n = 100: theoretical = -1.00
```
...
<Figure src="/courses/statistics-1-foundations/bias_demo-1.png" alt="Biased vs unbiased estimators of variance">
	Biased vs unbiased estimators of variance
</Figure>
...
```
##        n Var(X<U+0304>) Empirical Var(X<U+0304>) Theoretical SE Empirical
##    <num>                    <num>                      <num>        <num>
## 1:     5                    46.34                      45.00         6.81
## 2:    10                    22.58                      22.50         4.75
## 3:    25                     9.18                       9.00         3.03
## 4:    50                     4.46                       4.50         2.11
## 5:   100                     2.24                       2.25         1.50
## 6:   200                     1.16                       1.12         1.08
##    SE Theoretical
##             <num>
## 1:           6.71
## 2:           4.74
...
<Figure src="/courses/statistics-1-foundations/variance_estimator-1.png" alt="Variance of the sample mean decreases with sample size">
...
```
##   d = 9: Bias =  -0.15, Var = 2265.53, MSE = 2265.33
##   d = 10: Bias =  -9.61, Var = 1846.78, MSE = 1938.94
##   d = 11: Bias = -18.14, Var = 1513.06, MSE = 1841.87
```
...
```
## 
## Optimal divisor (minimising MSE): d = 10.5
```
...
<Figure src="/courses/statistics-1-foundations/mse_demo-1.png" alt="The bias-variance trade-off: MSE decomposes into variance plus squared bias">
	The bias-variance trade-off: MSE decomposes into variance plus squared bias
</Figure>
...
```
##        n P(|X<U+0304> - <U+03BC>| <U+2264> <U+03B5>) Mean of X<U+0304>
##    <num>                                       <num>             <num>
## 1:    10                                       0.486             50.05
## 2:    25                                       0.662             50.08
## 3:    50                                       0.859             50.03
...
<Figure src="/courses/statistics-1-foundations/consistency_demo-1.png" alt="Consistent estimators converge to the true parameter as n increases">
	Consistent estimators converge to the true parameter as n increases
</Figure>
...
```
## Efficiency Comparison for Normal Data
```
...
```
## True <U+03BC> = 100, <U+03C3> = 15, n = 50
```
...
```
##      Estimator   Mean Variance Rel. Eff.
##         <char>  <num>    <num>     <num>
## 1: Sample Mean  99.97     4.46     1.000
## 2: 10% Trimmed  99.95     4.75     0.939
## 3: 25% Trimmed 100.02     5.55     0.804
## 4:      Median 100.02     6.62     0.673
```
...
```
## Relative efficiency < 1 means higher variance (less efficient).
```
...
<Figure src="/courses/statistics-1-foundations/efficiency_demo-1.png" alt="Comparing efficiency: sample mean vs trimmed mean for normal data">
	Comparing efficiency: sample mean vs trimmed mean for normal data
</Figure>
...
```
## True parameters: <U+03BC> = 50, <U+03C3><U+00B2> = 100
```
...
```
##   <U+03BC><U+0302> = X<U+0304> = 50.527 (true: 50)
```
...
```
##   <U+03C3><U+0302><U+00B2> = (1/n)<U+03A3>(X<U+1D62> - X<U+0304>)<U+00B2> = 98.242 (true: 100)
```
...
```
## Comparison with unbiased variance:
```
...
```
##   S<U+00B2> = (1/(n-1))<U+03A3>(X<U+1D62> - X<U+0304>)<U+00B2> = 99.235
```
...
```
##   Ratio (MOM/S<U+00B2>) = 0.9900 <U+2248> (n-1)/n = 0.9900
```
...
```
## MOM Estimation for Exponential Distribution
```
...
```
## MOM estimator: <U+03BB><U+0302> = 1/X<U+0304>
```
...
```
## Sampling distribution of <U+03BB><U+0302> (n = 30):
```
...
```
##   Mean of <U+03BB><U+0302>: 0.5181 (true: 0.5)
```
...
```
##   Note: <U+03BB><U+0302> is biased! E(1/X<U+0304>) <U+2260> 1/E(X<U+0304>)
```
...
<Figure src="/courses/statistics-1-foundations/mom_exponential-1.png" alt="MOM estimation for exponential distribution">
	MOM estimation for exponential distribution
</Figure>
...
```
## MOM Estimation for Gamma Distribution
```
...
```
## True parameters: <U+03B1> = 3.0, <U+03B2> = 0.5
```
...
```
## True mean = <U+03B1>/<U+03B2> = 6.0, True variance = <U+03B1>/<U+03B2><U+00B2> = 12.0
```
...
```
##   <U+03B1><U+0302> = X<U+0304><U+00B2>/S<U+00B2> = 3.857 (true: 3.0)
```
...
```
##   <U+03B2><U+0302> = X<U+0304>/S<U+00B2> = 0.669 (true: 0.5)
```
...
```
## True diabetes prevalence: p = 0.0760 (7.60%)
```
...
```
## Sampling properties of prevalence estimator p<U+0302>:
```
...
```
##        n Mean p<U+0302>     Bias SE (emp) SE (theo)      MSE
##    <num>          <num>    <num>    <num>     <num>    <num>
## 1:    50         0.0757 -0.00025   0.0367    0.0375 0.001345
## 2:   100         0.0760  0.00002   0.0268    0.0265 0.000717
## 3:   200         0.0763  0.00027   0.0184    0.0187 0.000340
## 4:   500         0.0762  0.00020   0.0118    0.0119 0.000138
```
...
```
##   - p<U+0302> = X<U+0304> is unbiased (bias <U+2248> 0)
```
...
```
##   - Empirical SE matches theoretical <U+221A>[p(1-p)/n]
```
...
<Figure src="/courses/statistics-1-foundations/prevalence_application-1.png" alt="Estimating diabetes prevalence from NHANES with different estimators">
	Estimating diabetes prevalence from NHANES with different estimators
</Figure>
...
```
## Explaining Estimation to Non-Statisticians
```
...
```
##   'How do we use sample data to learn about the population?'
```
...
```
##   'An unbiased method gives the correct answer ON AVERAGE.
```
...
```
##    Like a scale that's properly calibrated <U+2014> individual weighings
```
...
```
##    might be a bit off, but there's no systematic over- or under-reading.'
```
...
```
##   'Precision tells us how tightly our estimates cluster.
```
...
```
##    More data = more precision. A study with 1000 patients gives
```
...
```
##    much more precise estimates than one with 50.'
```
...
```
##   'Overall accuracy combines bias and precision.
```
...
```
##    A method can be slightly biased but still accurate if it's
```
...
```
##    very precise. It's like archery: tight grouping matters,
```
...
```
##   'With enough data, we can get as close to the truth as we want.
```
...
```
##    This is reassuring: bigger studies DO give better answers.'
```
...
```
## Practical Implications of Estimator Properties
```
...
```
##   - Sample size affects precision (SE <U+221D> 1/<U+221A>n)
```
...
```
##   - Balance: larger studies are more precise but more expensive
```
...
```
##   - A single estimate is not the truth <U+2014> it has uncertainty
```
...
```
##   - Report standard errors alongside estimates
```
...
```
##   - Consider both bias and precision when evaluating methods
```
...
```
##   - Unbiased isn't always best (consider MSE)
```
...
```
##   - Robust methods may sacrifice efficiency for safety
```
...
```
##   - Context matters: what are the costs of different errors?
```
