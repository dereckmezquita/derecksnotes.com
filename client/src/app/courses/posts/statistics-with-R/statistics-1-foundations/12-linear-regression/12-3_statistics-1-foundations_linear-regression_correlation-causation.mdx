...
```
## r = S_xy / <U+221A>(S_xx <U+00D7> S_yy) = 0.4618
```
...
```
## Relationship Between r and <U+03B2><U+2081>
```
...
```
## r = <U+03B2><U+2081> <U+00D7> (s_x / s_y) = 0.9225 <U+00D7> (10.1049 / 20.1849) = 0.4618
```
...
![plot of chunk correlation_examples](/courses/statistics-1-foundations/correlation_examples-1.png)
...
```
## H<U+2080>: <U+03C1> = 0 (no linear relationship)
```
...
```
## H<U+2081>: <U+03C1> <U+2260> 0 (linear relationship exists)
```
...
```
## 95% Confidence Interval for <U+03C1>
```
...
![plot of chunk spurious_correlation](/courses/statistics-1-foundations/spurious_correlation-1.png)
...
```
##    Ice Cream  <U+2190><U+2192>  Drownings
```
...
```
## The observed correlation between ice cream and drownings is
```
...
```
## spurious<U+2014>it disappears when we control for temperature.
```
...
![plot of chunk simpsons_paradox](/courses/statistics-1-foundations/simpsons_paradox-1.png)
...
```
## This is NOT real change<U+2014>it's regression to the mean!
```
...
```
## The 'changes' are artifacts of selecting extreme observations.
```
...
![plot of chunk rtm_visual](/courses/statistics-1-foundations/rtm_visual-1.png)
...
```
## Individual-level correlation: r = 0.484
```
...
```
## State-level correlation: r = -0.409
```
...
```
## The aggregate correlation is much stronger!
```
...
```
## We cannot use state-level data to make inferences about individuals.
```
...
![plot of chunk sample_size_effect](/courses/statistics-1-foundations/sample_size_effect-1.png)
...
```
## R<U+00B2>: 0.000274 (0.0274% of variance explained)
```
...
![plot of chunk extrapolation](/courses/statistics-1-foundations/extrapolation-1.png)
...
```
##        EXECUTIVE SUMMARY: HEIGHT AND WEIGHT RELATIONSHIP
```
...
```
## in adults. Taller individuals tend to weigh more.
```
...
```
## <U+2022> For every additional centimetre of height, weight increases
```
...
```
##   by approximately 0.9 kg on average.
```
...
```
## <U+2022> This relationship explains about 21% of the variation
```
...
```
## <U+2022> We are 95% confident that the true effect is between
```
...
```
## Height is a meaningful predictor of weight, but other factors
```
...
```
## (diet, exercise, genetics) account for the remaining 79%.
```
...
```
## <U+2022> This is an ASSOCIATION, not proof of causation.
```
...
```
## <U+2022> Predictions are reliable only within the observed height range
```
...
```
## <U+2022> Individual variation is substantial; this describes averages.
```
