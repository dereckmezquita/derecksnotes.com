...
```
## One-Sample T-Test: Systolic Blood Pressure
```
...
```
## H<U+2081>: <U+03BC> <U+2260> 120 mmHg (two-sided)
```
...
```
## Sample mean: x<U+0304> = 117.56 mmHg
```
...
```
## Test statistic: t = (117.56 - 120) / 2.64 = -0.926
```
...
```
## Decision: Fail to reject H<U+2080> at <U+03B1> = 0.05
## Conclusion: Insufficient evidence that mean BP differs from 120 mmHg
```
...
## 
## data:  bp_sample
## t = -0.92558, df = 49, p-value = 0.3592
...
```
## Custom one_sample_t_test() Results:
```
...
<div class="figure">
<img src="/courses/statistics-1-foundations/one_sample_visual-1.png" alt="Visualising the one-sample t-test" width="100%" />
<p class="caption">Visualising the one-sample t-test</p>
</div>
...
```
## Two-Sample T-Test: Blood Pressure by Sex
```
...
```
## Males: n = 40, mean = 119.85, SD = 18.12
```
...
```
## Females: n = 40, mean = 115.20, SD = 17.66
```
...
## 
## data:  male_bp and female_bp
## t = 1.1623, df = 77.947, p-value = 0.2487
...
```
## Custom two_sample_t_test() Results:
```
...
```
##   95% CI for difference: (-3.31, 12.61)
```
...
<div class="figure">
<img src="/courses/statistics-1-foundations/two_sample_visual-1.png" alt="Comparing two groups" width="100%" />
<p class="caption">Comparing two groups</p>
</div>
...
```
## Two-Sample T-Test (incorrect for paired data):
```
...
```
## Key insight: The paired test is much more powerful because
```
...
```
## it removes between-subject variability.
```
...
```
## 
## Correlation between before and after: r = 0.965
```
...
<div class="figure">
<img src="/courses/statistics-1-foundations/paired_visual-1.png" alt="Visualising paired data" width="100%" />
<p class="caption">Visualising paired data</p>
</div>
...
<div class="figure">
<img src="/courses/statistics-1-foundations/normality_check-1.png" alt="Checking normality assumption" width="100%" />
<p class="caption">Checking normality assumption</p>
</div>
...
```
## Result: No significant departure from normality
```
...
```
## Type I Error Rate Check (<U+03B1> = 0.05)
```
...
```
## Normal data: 0.0524 (should be <U+2248> 0.05)
```
...
```
## Exponential data: 0.0756 (should be <U+2248> 0.05)
```
...
<div class="figure">
<img src="/courses/statistics-1-foundations/robustness_demo-1.png" alt="T-test is robust to moderate non-normality" width="100%" />
<p class="caption">T-test is robust to moderate non-normality</p>
</div>
...
```
## Hedges' g = 0.2574 (bias-corrected)
```
...
```
## Effect Size with 95% CI (Bootstrap)
```
...
```
## STATISTICAL REPORT: Blood Pressure by Sex
```
...
```
## Male: M = 119.8 mmHg (SD = 18.1), n = 40
```
...
```
## Female: M = 115.2 mmHg (SD = 17.7), n = 40
```
...
```
## 95% CI for difference: [-3.3, 12.6] mmHg
```
...
```
## However, the effect size suggests the practical
```
