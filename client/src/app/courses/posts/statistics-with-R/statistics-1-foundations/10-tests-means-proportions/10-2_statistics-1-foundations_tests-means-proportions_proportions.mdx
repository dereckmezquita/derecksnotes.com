...
```
## One-Proportion Z-Test: Hypertension Prevalence
```
...
```
## Sample: 981 with high BP out of 8237 total
```
...
```
## Hypothesised proportion: p<U+2080> = 0.30
```
...
```
##   np<U+2080> = 2471.1 (should be <U+2265> 10)
```
...
```
##   n(1-p<U+2080>) = 5765.9 (should be <U+2265> 10)
```
...
```
## Standard error (under H<U+2080>): SE = <U+221A>[p<U+2080>(1-p<U+2080>)/n] = 0.0050
```
...
```
## Test statistic: z = (0.1191 - 0.30) / 0.0050 = -35.828
```
...
```
## Decision: Reject H<U+2080> at <U+03B1> = 0.05
## Conclusion: Proportion differs significantly from 30%
```
...
```
## Custom one_prop_z_test() Results:
```
...
## 
## data:  high_bp out of n, null probability 0.3
## X-squared = 1283.6, df = 1, p-value < 2.2e-16
...
<div class="figure">
<img src="/courses/statistics-1-foundations/one_prop_visual-1.png" alt="One-proportion z-test visualisation" width="100%" />
<p class="caption">One-proportion z-test visualisation</p>
</div>
...
```
## Exact Binomial Test: Coin Fairness
```
...
```
## Custom exact_binomial_test() Results:
```
...
```
## 95% CI (Clopper-Pearson): (0.4439, 0.9748)
```
...
```
## Two-Proportion Z-Test: Hypertension by Smoking Status
```
...
```
## Smokers: 146/1369 with high BP (10.7%)
```
...
```
## Non-smokers: 835/6868 with high BP (12.2%)
```
...
```
## Pooled proportion: p<U+0302> = 0.1191
```
...
```
## 95% CI for difference: (-0.0330, 0.0032)
```
...
```
## Decision: Fail to reject H<U+2080>
## Conclusion: No significant difference in hypertension rates
```
...
```
## Custom two_prop_z_test() Results:
```
...
## X-squared = 2.4256, df = 1, p-value = 0.1194
## alternative hypothesis: two.sided
## 95 percent confidence interval:
##  -0.033016368  0.003154075
## sample estimates:
##    prop 1    prop 2 
## 0.1066472 0.1215783
```
...
<div class="figure">
<img src="/courses/statistics-1-foundations/two_prop_visual-1.png" alt="Comparing two proportions" width="100%" />
<p class="caption">Comparing two proportions</p>
</div>
...
```
## Risk Difference: -0.0149 (-0.0330 to 0.0032)
```
...
```
## Relative Risk: 0.877 (0.743 to 1.036)
```
...
```
## Odds Ratio: 0.863 (0.716 to 1.039)
```
...
```
## Smokers are 12.3% less likely to have high BP
```
...
```
## Comparison of RR and OR at Different Baseline Risks
```
...
```
## (Group 1 has exactly double the risk of Group 2)
```
...
##       p1    p2    RR       OR
##    <num> <num> <num>    <num>
## 1:  0.02  0.01     2 2.020408
## 2:  0.10  0.05     2 2.111111
## 3:  0.20  0.10     2 2.250000
## 4:  0.40  0.20     2 2.666667
## 5:  0.60  0.30     2 3.500000
## 6:  1.00  0.50     2      Inf
```
...
```
## Fisher's Exact Test: Treatment Response
```
...
```
## Chi-square test (may be unreliable):
```
...
```
## Sample Size Calculations for Proportion Tests
```
...
```
## One-proportion test (detecting p = 0.35 vs H<U+2080>: p = 0.30):
```
...
```
##   n = 676 (80% power, <U+03B1> = 0.05, two-sided)
```
...
```
## Two-proportion test (detecting p<U+2081> = 0.35 vs p<U+2082> = 0.25):
```
...
```
##   n = 329 per group (80% power, <U+03B1> = 0.05, two-sided)
```
...
```
## Verification with power.prop.test():
```
...
```
## STATISTICAL REPORT: Hypertension by Smoking Status
```
...
```
## Smokers: 146/1369 (10.7%) had high blood pressure
```
...
```
## Non-smokers: 835/6868 (12.2%) had high blood pressure
```
...
```
## Risk difference: -1.5 percentage points (95% CI: -3.3 to 0.3)
```
...
```
## Relative risk: 0.88 (95% CI: 0.74 to 1.04)
```
...
```
## Two-proportion z-test: z = -1.56, p = 0.119
```
...
```
## There was no statistically significant difference in hypertension
## prevalence between smokers and non-smokers.
```
