...
```{r overfitting_demo, fig.cap="Training error decreases with complexity; test error has a minimum (optimal complexity)"}
...
```{r kfold_visual, fig.cap="5-fold cross-validation: each fold serves as validation once"}
...
```{r k_comparison, fig.cap="Effect of K on CV estimate variance and bias"}
...
```{r k_boxplot, fig.cap="CV estimate distributions for different K values"}
...
```{r stratified_cv, fig.cap="Stratified CV preserves class proportions in each fold"}
...
```{r cv_model_selection, fig.cap="Using CV to select the optimal regularisation parameter lambda"}
