...
```{r compositional_bias_demo, fig.cap="Compositional bias: apparent changes may be artefacts"}
...
```{r method_comparison, fig.cap="Comparing differential abundance methods"}
# Combine results
...
```{r effect_size_agreement, fig.cap="Effect size correlation between methods"}
...
```{r volcano_plot_microbiome, fig.cap="Volcano plot of differential abundance results"}
...
```{r abundance_barplot, fig.cap="Relative abundance profiles by group"}
...
```{r significant_taxa_boxplot, fig.cap="Abundance distributions of top differential taxa"}
