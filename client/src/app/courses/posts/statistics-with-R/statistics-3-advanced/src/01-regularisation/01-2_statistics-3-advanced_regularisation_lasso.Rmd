...
```{r lasso_vs_ridge_geometry, fig.cap="L1 (LASSO) constraint has corners promoting sparsity; L2 (ridge) is smooth"}
...
```{r lasso_path, fig.cap="LASSO shrinks coefficients to exactly zero as λ increases"}
...
```{r lasso_selection, fig.cap="Number of non-zero coefficients decreases with regularisation"}
...
```{r lasso_cv, fig.cap="Cross-validation identifies optimal LASSO regularisation"}
set.seed(42)
...
```{r soft_threshold, fig.cap="Soft thresholding shrinks small coefficients to zero"}
...
```{r prior_comparison, fig.cap="Laplace prior (LASSO) concentrates more mass at zero than Gaussian (ridge)"}
...
```{r comparison_simulation, fig.cap="LASSO excels with sparse truth; ridge excels when all predictors matter"}
# Simulate comparison
set.seed(42)
n <- 200
p <- 20
...
```{r publication_figure_lasso, fig.cap="LASSO variable selection for breast cancer diagnosis", fig.height=10}
library(patchwork)
