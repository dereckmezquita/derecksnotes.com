...
```{r simulate_clinical_data, message=FALSE}
...
```{r pooling_comparison, fig.cap="Complete pooling ignores structure; no pooling gives noisy estimates"}
...
```{r bayes_demo, fig.cap="Bayesian updating: prior + data = posterior"}
...
```{r metropolis_demo, fig.cap="Metropolis algorithm samples from the posterior"}
...
```{r mcmc_diagnostics, fig.cap="MCMC diagnostics: trace plot, autocorrelation, and density"}
# Run multiple chains
...
```{r proposal_tuning, fig.cap="Proposal variance affects mixing and acceptance rate"}
...
```{r shrinkage_visualisation, fig.cap="Shrinkage: hierarchical estimates are pulled toward the grand mean"}
