...
```{r elastic_net_constraint, fig.cap="Elastic net constraint region interpolates between L1 diamond and L2 circle"}
...
```{r elastic_net_paths, fig.cap="Coefficient paths vary with alpha: ridge shrinks smoothly, LASSO creates sharp zeros"}
...
```{r grouping_effect, fig.cap="LASSO selects one correlated predictor; elastic net keeps groups together"}
...
```{r alpha_selection_plot, fig.cap="Model performance across alpha values; higher alpha gives sparser models"}
...
```{r roc_curves, fig.cap="ROC curves for the three regularisation methods on held-out test data"}
# Generate ROC data
...
```{r feature_comparison, fig.cap="Features selected by each method; LASSO and elastic net select sparse subsets"}
...
6. **Don't over-interpret zero coefficients** — they may be zero due to correlation, not irrelevance
