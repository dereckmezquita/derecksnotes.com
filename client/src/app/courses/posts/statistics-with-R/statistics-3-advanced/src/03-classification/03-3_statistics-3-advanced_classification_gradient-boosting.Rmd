...
```{r boosting_intuition, fig.cap="Boosting builds models sequentially, each correcting previous errors"}
...
```{r training_curve, fig.cap="Training and validation loss—early stopping prevents overfitting"}
...
```{r regularisation_comparison, fig.cap="Stronger regularisation (lower eta) requires more rounds but generalises better"}
...
```{r xgb_importance, fig.cap="Variable importance in XGBoost based on gain"}
# Get importance
...
```{r shap_values, fig.cap="SHAP values show how each feature contributes to individual predictions"}
...
```{r grid_search, fig.cap="Grid search over max_depth and eta to find optimal hyperparameters"}
# Simple grid search
...
```{r model_comparison, fig.cap="Comparison of decision tree, random forest, and gradient boosting"}
