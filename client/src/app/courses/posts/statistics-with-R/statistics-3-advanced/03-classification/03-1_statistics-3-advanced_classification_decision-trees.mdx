...
<Figure src="/courses/statistics-3-advanced/tree_intuition-1.png" alt="Decision trees partition feature space with axis-aligned boundaries">
	Decision trees partition feature space with axis-aligned boundaries
</Figure>
...
<Figure src="/courses/statistics-3-advanced/impurity_measures-1.png" alt="Gini and entropy are more sensitive to class imbalance than misclassification error">
	Gini and entropy are more sensitive to class imbalance than misclassification error
</Figure>
...
#> 
#>  1) root 398 153 Benign (0.61557789 0.38442211)  
#>    2) mean_concave_points< 0.04892 241  13 Benign (0.94605809 0.05394191)  
#>      4) mean_radius< 15.025 230   6 Benign (0.97391304 0.02608696) *
#>      5) mean_radius>=15.025 11   4 Malignant (0.36363636 0.63636364)  
#>       10) mean_smoothness< 0.08251 5   1 Benign (0.80000000 0.20000000) *
#>       11) mean_smoothness>=0.08251 6   0 Malignant (0.00000000 1.00000000) *
#>    3) mean_concave_points>=0.04892 157  17 Malignant (0.10828025 0.89171975)  
#>      6) mean_texture< 15.425 15   4 Benign (0.73333333 0.26666667)  
#>       12) mean_perimeter< 102.25 10   0 Benign (1.00000000 0.00000000) *
#>       13) mean_perimeter>=102.25 5   1 Malignant (0.20000000 0.80000000) *
#>      7) mean_texture>=15.425 142   6 Malignant (0.04225352 0.95774648)  
#>       14) mean_area< 525.1 11   4 Malignant (0.36363636 0.63636364)  
#>         28) mean_smoothness< 0.10895 5   1 Benign (0.80000000 0.20000000) *
#>         29) mean_smoothness>=0.10895 6   0 Malignant (0.00000000 1.00000000) *
#>       15) mean_area>=525.1 131   2 Malignant (0.01526718 0.98473282) *
```
...
<Figure src="/courses/statistics-3-advanced/tree_plot-1.png" alt="Decision tree for breast cancer diagnosis—interpretable clinical rules">
	Decision tree for breast cancer diagnosis—interpretable clinical rules
</Figure>
...
```
#> Root Node Split Selection:
#> ==========================
#> 
#> Before split:
#>   P(Malignant) = 0.384 
#>   Gini impurity = 0.4733 
#> 
#> Candidate splits and weighted Gini after split:
#>   mean_radius <= 15.13: weighted Gini = 0.1971, reduction = 0.2762
#>   mean_concave_points <= 0.05: weighted Gini = 0.1458, reduction = 0.3274
#>   mean_area <= 712.70: weighted Gini = 0.1889, reduction = 0.2844
```
...
<Figure src="/courses/statistics-3-advanced/complexity_plot-1.png" alt="Cross-validation error vs complexity parameter; optimal pruning minimises CV error">
	Cross-validation error vs complexity parameter; optimal pruning minimises CV error
</Figure>
...
#>          <num>  <num>      <num>     <num>      <num>
#> 1: 0.803921569      0 1.00000000 1.0000000 0.06343016
#> 2: 0.045751634      1 0.19607843 0.2156863 0.03595592
#> 3: 0.019607843      2 0.15032680 0.1699346 0.03221997
#> 4: 0.009803922      5 0.09150327 0.1764706 0.03278958
#> 5: 0.001000000      7 0.07189542 0.1633987 0.03163672
```
...
<Figure src="/courses/statistics-3-advanced/prune_tree-1.png" alt="Pruned tree retains key splits while removing overfitting branches">
	Pruned tree retains key splits while removing overfitting branches
</Figure>

```
#> Pruning Decision:
#> =================
#>   Minimum CV error: 0.1634 
#>   1-SE threshold: 0.195 
#>   Optimal cp: 0.01960784 
#> 
#>   Full tree leaves: 8 
#>   Pruned tree leaves: 3
```
...
<Figure src="/courses/statistics-3-advanced/roc_curve-1.png" alt="ROC curve for pruned decision tree">
...
<Figure src="/courses/statistics-3-advanced/variable_importance-1.png" alt="Variable importance—total impurity reduction from each feature">
	Variable importance—total impurity reduction from each feature
</Figure>
...
```
#> Root Node Variable Across Bootstrap Samples:
#> =============================================
#>   Bootstrap 1: mean_concave_points
#>   Bootstrap 2: mean_concave_points
#>   Bootstrap 3: mean_concave_points
#>   Bootstrap 4: mean_concave_points
```
...
#> 
#> Rule 2 (n = 15):
#>   IF mean_concave_points < 0.05 AND <leaf> < NA 
#>   THEN predict: Benign 
#> 
#> Rule 3 (n = 142):
#>   IF mean_concave_points < 0.05 AND <leaf> >= NA 
