...
#> 
#> Optimal λ: 0 
#> 95% CI: Check if λ = 0 (log) or λ = 1 (no transform) is included
```
...
#>   R² = 0.1733 
#> 
#> Quadratic model: SBP ~ Age + Age²
#>   R² = 0.1805 
#>   Age coefficient: -0.053 
#>   Age² coefficient: 0.00477 
#> 
#> F-test for quadratic term:
#>   F = 63.52 , p = 1.836e-15
```
...
#> =============================
#> 
#> Model: log(e²) ~ log(fitted)
#> Coefficient: 2.262 
#> If coefficient ≈ 2, variance is proportional to fitted²
#> 
#> FGLS Coefficients:
#>              Estimate Std. Error   t value      Pr(>|t|)
...
#>   → Transform Y (log, sqrt) if variance increases with mean
#>   → Use weighted least squares
#>   → Use robust/sandwich standard errors
#> 
#> PROBLEM: Nonlinearity
#>   → Transform X (log, polynomial)
#>   → Transform Y
#>   → Use GAMs (generalized additive models)
#> 
#> PROBLEM: Outliers
#>   → Investigate cause
#>   → Use robust regression (M-estimation, LTS)
#>   → Sensitivity analysis (with/without)
#> 
#> PROBLEM: Non-normal residuals
#>   → With large n: Usually not a concern (CLT)
#>   → Transform Y
#>   → Use bootstrap for inference
#> 
#> PROBLEM: Independence violation
#>   → Clustered data: Use mixed-effects models (Chapter 6)
#>   → Time series: Use time series methods (Chapter 7)
#>   → Spatial data: Use spatial models (Part III)
```
