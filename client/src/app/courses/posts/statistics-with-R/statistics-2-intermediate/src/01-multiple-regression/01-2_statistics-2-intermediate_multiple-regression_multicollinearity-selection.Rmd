...
```{r collinearity_example, fig.cap="Demonstrating the effect of correlated predictors"}
...
```{r variance_inflation, fig.cap="How collinearity inflates variance"}
...
```{r correlation_matrix, fig.cap="Correlation heatmap of predictors"}
...
```{r vif_calculation, fig.cap="Computing and interpreting VIF"}
...
```{r remove_redundant, fig.cap="Comparing models with and without redundant predictors"}
...
```{r combine_predictors, fig.cap="Using principal components to address collinearity"}
...
```{r aic_bic, fig.cap="Comparing models using AIC and BIC"}
...
```{r stepwise, fig.cap="Stepwise selection in action"}
# Full model
...
```{r cross_validation, fig.cap="K-fold cross-validation for model selection"}
...
```{r partial_correlation, fig.cap="Partial correlation removes shared variance"}
# Simple correlations
