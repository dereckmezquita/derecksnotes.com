...
```{r logistic_likelihood, fig.cap="Log-likelihood surface for logistic regression"}
...
```{r irls_implementation, fig.cap="IRLS convergence for logistic regression"}
...
```{r wald_inference, fig.cap="Wald-based inference for logistic regression"}
...
```{r lrt, fig.cap="Likelihood ratio test comparing nested models"}
# Compare models
...
```{r deviance, fig.cap="Deviance analysis for model fit"}
# Deviance components
...
```{r overdispersion, fig.cap="Checking for overdispersion"}
...
```{r aic_bic_glm, fig.cap="Model comparison using AIC"}
...
1. **Age effect**: For each additional year of age, the odds of having diabetes increase by about `r round((exp(coef(model_full)["Age"]) - 1) * 100, 1)`% (95% CI: `r round((exp(confint(model_full)["Age", 1]) - 1) * 100, 1)`% to `r round((exp(confint(model_full)["Age", 2]) - 1) * 100, 1)`%).

2. **BMI effect**: For each additional unit of BMI, the odds increase by about `r round((exp(coef(model_full)["BMI"]) - 1) * 100, 1)`%.
