...
```{r censoring_illustration, fig.cap="Censoring occurs when we don't observe the true event time"}
...
```{r hazard_shapes, fig.cap="Different hazard functions imply different survival patterns"}
...
```{r km_calculation, fig.cap="Step-by-step Kaplan-Meier calculation"}
...
```{r km_lung, fig.cap="Kaplan-Meier survival curve for lung cancer patients"}
# Fit Kaplan-Meier
...
```{r km_with_risk_table, fig.cap="Kaplan-Meier curve with number at risk table"}
...
```{r logrank_sex, fig.cap="Kaplan-Meier curves comparing survival by sex with log-rank test"}
# Fit KM by sex
