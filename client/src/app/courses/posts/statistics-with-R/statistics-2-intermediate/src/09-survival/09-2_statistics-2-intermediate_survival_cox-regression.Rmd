...
```{r proportional_hazards_illustration, fig.cap="Proportional hazards means survival curves don't cross and hazard ratio is constant"}
...
```{r forest_plot, fig.cap="Forest plot of hazard ratios from the Cox model"}
...
```{r schoenfeld_plot, fig.cap="Scaled Schoenfeld residuals should show no trend with time under PH"}
...
```{r loglog_plot, fig.cap="Parallel log-log survival curves support the PH assumption"}
# Fit KM by sex
...
```{r residual_diagnostics, fig.cap="Residual diagnostics for the Cox model"}
# Calculate residuals
...
```{r influential, fig.cap="Identifying influential observations in the Cox model"}
...
```{r predicted_survival, fig.cap="Predicted survival curves for specific patient profiles"}
