...
```{r raw_residuals, fig.cap="Distribution of raw residuals"}
...
```{r standardised_residuals, fig.cap="Standardised residuals adjust for leverage"}
...
```{r resid_vs_fitted, fig.cap="Residuals vs fitted values plot"}
# Residuals vs fitted
...
```{r qq_plot, fig.cap="Normal Q-Q plot of standardised residuals"}
# Q-Q plot
...
```{r scale_location, fig.cap="Scale-location plot for homoscedasticity"}
# Scale-location plot
...
```{r resid_vs_predictors, fig.cap="Residuals vs individual predictors"}
...
```{r leverage, fig.cap="Identifying high-leverage observations"}
# Leverage analysis
n <- nrow(bp_data)
...
```{r outliers, fig.cap="Identifying outliers by studentised residuals"}
# Outlier analysis
...
```{r cooks_distance, fig.cap="Cook's distance identifies influential points"}
...
```{r influence_plot, fig.cap="Influence plot combining leverage, residuals, and Cook's distance"}
# Influence plot
...
```{r dfbetas, fig.cap="DFBETAS shows influence on individual coefficients"}
# Compute DFBETAS
...
```{r hospital_example, fig.cap="Diagnostic analysis of hospital length of stay model"}
...
```{r hospital_diagnostics, fig.cap="Residual plots reveal problems with the hospital LOS model"}
...
```{r hospital_qq, fig.cap="Q-Q plot shows severe non-normality in hospital LOS residuals"}
print(p2)
```

```{r hospital_interpretation}
