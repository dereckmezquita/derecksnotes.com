...
```{r stationary_nonstationary, fig.cap="Comparison of stationary and non-stationary time series processes"}
set.seed(42)
n <- 200
...
```{r adf_test, fig.cap="ADF test demonstrates random walk is non-stationary"}
...
```{r differencing_example, fig.cap="Differencing transforms non-stationary random walk to stationary white noise"}
...
```{r ar_processes, fig.cap="AR(1) processes with different coefficients show varying persistence"}
set.seed(123)
n <- 200
...
```{r ar_acf_pacf, fig.cap="AR(2) process shows exponentially decaying ACF and PACF cutoff at lag 2"}
...
```{r ma_processes, fig.cap="MA(1) processes with different coefficients"}
set.seed(456)
n <- 200
...
```{r ma_acf_pacf, fig.cap="MA(2) process shows sharp ACF cutoff and exponentially decaying PACF"}
...
```{r boxjenkins_flow, fig.cap="The Box-Jenkins methodology for ARIMA modelling"}
...
```{r hospital_ts, fig.cap="Hospital admissions show clear upward trend and potential seasonality"}
...
```{r hospital_stationarity, fig.cap="ACF of hospital admissions shows slow decay typical of non-stationarity"}
# Test stationarity
...
```{r hospital_diff, fig.cap="First differenced hospital admissions appear stationary"}
# First difference
...
```{r hospital_acf_pacf_diff, fig.cap="ACF and PACF of differenced series suggest MA(1) or ARIMA(0,1,1)"}
...
```{r residual_diagnostics, fig.cap="Residual diagnostics for the ARIMA(0,1,1) model"}
...
```{r sarima_deaths, fig.cap="US accidental deaths show strong seasonal pattern requiring SARIMA"}
# Create time series
...
```{r sarima_seasonal_diff, fig.cap="Seasonal differencing removes the annual cycle"}
...
```{r sarima_diagnostics, fig.cap="SARIMA model residuals appear to be white noise"}
...
```{r stakeholder_forecast, fig.cap="12-month forecast of US accidental deaths with prediction intervals"}
if (requireNamespace("forecast", quietly = TRUE)) {
    # Generate forecast
...
```{r pattern_summary, fig.cap="Summary of ACF/PACF patterns for common time series models"}
patterns <- data.table(
