...
```{r philosophy_comparison, fig.cap="Frequentist vs Bayesian interpretation of the same data"}
...
```{r bayesian_perspective, fig.cap="Bayesian perspective: posterior distribution represents uncertainty about the parameter"}
...
```{r bayes_tree, fig.cap="Visualising Bayes' theorem with a probability tree"}
...
```{r prior_types, fig.cap="Different types of prior distributions for a probability parameter"}
...
```{r conjugate_example, fig.cap="Beta-Binomial conjugacy: prior to posterior updating"}
...
```{r sample_size_effect, fig.cap="With more data, the posterior is increasingly dominated by the likelihood"}
# True response rate
true_theta <- 0.65
...
```{r point_estimates, fig.cap="Different point estimates from a skewed posterior"}
...
```{r credible_intervals, fig.cap="Equal-tailed interval vs highest density interval for a skewed posterior"}
...
```{r prior_elicitation, fig.cap="Prior elicitation for vaccine efficacy based on historical data"}
...
```{r sensitivity_analysis, fig.cap="Sensitivity analysis: how different priors affect the posterior"}
