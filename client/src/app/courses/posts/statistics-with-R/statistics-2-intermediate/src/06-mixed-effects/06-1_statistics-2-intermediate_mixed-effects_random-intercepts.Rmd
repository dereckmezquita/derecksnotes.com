...
```{r hierarchical_illustration, fig.cap="Illustration of hierarchical data structure"}
...
```{r random_intercept_illustration, fig.cap="Random intercepts allow different baselines for each patient"}
...
```{r icc_illustration, fig.cap="Visualising what ICC means"}
...
```{r patient_predictions, fig.cap="Mixed models provide patient-specific predictions"}
...
```{r shrinkage, fig.cap="BLUPs are shrunk toward zero compared to raw means"}
...
```{r diagnostics, fig.cap="Diagnostic plots for mixed-effects models"}
# Extract residuals
...
```{r diagnostics_qq, fig.cap="Q-Q plots for checking normality assumptions"}
...
```{r diagnostics_re, fig.cap="Q-Q plot for random effects should be approximately normal"}
print(p3)
```
