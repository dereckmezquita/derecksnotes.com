...
#>   Doctor Visits (NMES): 4406 observations
```
...
<Figure src="/courses/statistics-2-intermediate/problems_illustration-1.png" alt="Why linear regression fails for binary outcomes">
...
<Figure src="/courses/statistics-2-intermediate/exponential_family-1.png" alt="Variance functions for different exponential family distributions">
	Variance functions for different exponential family distributions
</Figure>
...
#>              <char>                <char>               <char>
#> 1:           Normal   Identity (<U+03BC>)         1 (constant)
#> 2:         Binomial  Logit (log(p/(1-p))) <U+03BC>(1-<U+03BC>)
#> 3:          Poisson    Log (log <U+03BB>)             <U+03BC>
#> 4:            Gamma Inverse (-1/<U+03BC>)     <U+03BC><U+00B2>
#> 5: Inverse Gaussian   -1/<U+03BC><U+00B2>     <U+03BC><U+00B3>
#>             Common Use
#>                 <char>
#> 1:          Continuous
#> 2:  Binary/Proportions
#> 3:              Counts
#> 4: Positive continuous
#> 5: Positive continuous
```
...
<Figure src="/courses/statistics-2-intermediate/link_functions-1.png" alt="Common link functions and their inverse">
	Common link functions and their inverse
</Figure>
...
#> The canonical link is special because <U+03B8> = <U+03B7>.
cat("Distribution   Canonical Link    Why?\n")
#> Distribution   Canonical Link    Why?
cat("-----------    --------------    ----\n")
#> -----------    --------------    ----
cat("Normal         g(μ) = μ          θ = μ for normal\n")
#> Normal         g(<U+03BC>) = <U+03BC>          <U+03B8> = <U+03BC> for normal
cat("Binomial       g(μ) = logit(μ)   θ = log(μ/(1-μ))\n")
#> Binomial       g(<U+03BC>) = logit(<U+03BC>)   <U+03B8> = log(<U+03BC>/(1-<U+03BC>))
cat("Poisson        g(μ) = log(μ)     θ = log(μ)\n")
#> Poisson        g(<U+03BC>) = log(<U+03BC>)     <U+03B8> = log(<U+03BC>)
cat("Gamma          g(μ) = -1/μ       θ = -1/μ\n")
#> Gamma          g(<U+03BC>) = -1/<U+03BC>       <U+03B8> = -1/<U+03BC>
```
...
#> GLM (Logistic) vs Linear Regression:
cat("====================================\n\n")
#> ====================================
...
<Figure src="/courses/statistics-2-intermediate/glm_example-1.png" alt="Comparing linear model to logistic GLM">
	Comparing linear model to logistic GLM
</Figure>
...
#> Variance of visits: 45.69
cat("Note: Variance >> Mean suggests overdispersion\n\n")
#> Note: Variance >> Mean suggests overdispersion
...
print(coef(model_linear))
#> (Intercept)         age health_poor 
#>   6.4550274  -0.1531174   3.6012166
...
#> (Intercept)         age health_poor 
#>  1.86621086 -0.02627672  0.51827109
cat("\nRate ratios:\n")
#> 
#> Rate ratios:
...
#>   Interpretation: Poor health multiplies expected visits by 1.68
```
...
#> 
#> Call:
#> glm(formula = visits ~ age + health_poor, family = poisson, data = visits_data)
#> 
#> Coefficients:
#>             Estimate Std. Error z value Pr(>|z|)    
#> (Intercept)  1.86621    0.07357  25.367   <2e-16 ***
#> age         -0.02628    0.00994  -2.644   0.0082 ** 
#> health_poor  0.51827    0.01597  32.444   <2e-16 ***
#> ---
...
#> Residual deviance: 25996  on 4403  degrees of freedom
#> AIC: 38778
#> 
...
#> 1. RESPONSE TYPE: Binary (0/1, yes/no, success/failure)
cat("   → Family: binomial\n")
#>    <U+2192> Family: binomial
cat("   → Links: logit (most common), probit, cloglog\n")
#>    <U+2192> Links: logit (most common), probit, cloglog
cat("   → Examples: Disease status, treatment response, mortality\n\n")
#>    <U+2192> Examples: Disease status, treatment response, mortality

cat("2. RESPONSE TYPE: Counts (0, 1, 2, 3, ...)\n")
#> 2. RESPONSE TYPE: Counts (0, 1, 2, 3, ...)
cat("   → Family: poisson (if variance ≈ mean)\n")
#>    <U+2192> Family: poisson (if variance <U+2248> mean)
cat("   → Family: quasipoisson or negative.binomial (if variance > mean)\n")
#>    <U+2192> Family: quasipoisson or negative.binomial (if variance > mean)
cat("   → Link: log\n")
#>    <U+2192> Link: log
cat("   → Examples: Doctor visits, mutations, adverse events\n\n")
#>    <U+2192> Examples: Doctor visits, mutations, adverse events

cat("3. RESPONSE TYPE: Positive continuous (costs, times, concentrations)\n")
#> 3. RESPONSE TYPE: Positive continuous (costs, times, concentrations)
cat("   → Family: Gamma (if variance ∝ mean²)\n")
#>    <U+2192> Family: Gamma (if variance <U+221D> mean<U+00B2>)
cat("   → Link: log (most interpretable) or inverse (canonical)\n")
#>    <U+2192> Link: log (most interpretable) or inverse (canonical)
cat("   → Examples: Hospital costs, survival times, drug concentrations\n\n")
#>    <U+2192> Examples: Hospital costs, survival times, drug concentrations

cat("4. RESPONSE TYPE: Continuous, unbounded\n")
#> 4. RESPONSE TYPE: Continuous, unbounded
cat("   → Family: gaussian (= normal linear regression)\n")
#>    <U+2192> Family: gaussian (= normal linear regression)
cat("   → Link: identity\n")
#>    <U+2192> Link: identity
cat("   → Examples: Blood pressure, BMI, test scores\n\n")
#>    <U+2192> Examples: Blood pressure, BMI, test scores

cat("5. RESPONSE TYPE: Proportion (between 0 and 1)\n")
#> 5. RESPONSE TYPE: Proportion (between 0 and 1)
cat("   → Family: quasibinomial or beta regression\n")
#>    <U+2192> Family: quasibinomial or beta regression
cat("   → Link: logit\n")
#>    <U+2192> Link: logit
cat("   → Examples: Percentage of cells responding\n")
#>    <U+2192> Examples: Percentage of cells responding
```
