...
#> ===========================
#> 
#>        Distribution       Canonical Link Variance Function          Common Use
#>              <char>               <char>            <char>              <char>
#> 1:           Normal         Identity (μ)      1 (constant)          Continuous
#> 2:         Binomial Logit (log(p/(1-p)))            μ(1-μ)  Binary/Proportions
#> 3:          Poisson          Log (log λ)                 μ              Counts
#> 4:            Gamma       Inverse (-1/μ)                μ² Positive continuous
#> 5: Inverse Gaussian                -1/μ²                μ³ Positive continuous
```
...
#> ================
#> 
#> The canonical link is special because θ = η.
#> 
#> Distribution   Canonical Link    Why?
#> -----------    --------------    ----
#> Normal         g(μ) = μ          θ = μ for normal
#> Binomial       g(μ) = logit(μ)   θ = log(μ/(1-μ))
#> Poisson        g(μ) = log(μ)     θ = log(μ)
#> Gamma          g(μ) = -1/μ       θ = -1/μ
```
...
#>    → Family: binomial
#>    → Links: logit (most common), probit, cloglog
#>    → Examples: Disease status, treatment response, mortality
#> 
#> 2. RESPONSE TYPE: Counts (0, 1, 2, 3, ...)
#>    → Family: poisson (if variance ≈ mean)
#>    → Family: quasipoisson or negative.binomial (if variance > mean)
#>    → Link: log
#>    → Examples: Doctor visits, mutations, adverse events
#> 
#> 3. RESPONSE TYPE: Positive continuous (costs, times, concentrations)
#>    → Family: Gamma (if variance ∝ mean²)
#>    → Link: log (most interpretable) or inverse (canonical)
#>    → Examples: Hospital costs, survival times, drug concentrations
#> 
#> 4. RESPONSE TYPE: Continuous, unbounded
#>    → Family: gaussian (= normal linear regression)
#>    → Link: identity
#>    → Examples: Blood pressure, BMI, test scores
#> 
#> 5. RESPONSE TYPE: Proportion (between 0 and 1)
#>    → Family: quasibinomial or beta regression
#>    → Link: logit
#>    → Examples: Percentage of cells responding
```
