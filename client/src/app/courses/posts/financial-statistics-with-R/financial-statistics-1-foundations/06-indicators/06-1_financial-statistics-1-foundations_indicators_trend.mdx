...
![Different moving averages applied to the same price series. EMA responds faster but is noisier than SMA.](/courses/financial-statistics-1-foundations/../figures/06-1_ma-comparison-1.png)
...
![Moving average lag: the average trails the price, responding to changes with delay. Longer MAs have more lag.](/courses/financial-statistics-1-foundations/../figures/06-1_ma-lag-analysis-1.png)
...
![Moving average crossover signals. Golden cross (fast crosses above slow) signals uptrend; death cross signals downtrend.](/courses/financial-statistics-1-foundations/../figures/06-1_ma-crossover-1.png)
...
```
## Smoothness (variance of daily changes):
```
...
```
## Period: 2010-10-19 to 2023-12-29
```
...
## 1:    Buy & Hold       13.2%    17.3%   0.72   NaN%
## 2: SMA Crossover        9.0%    14.2%   0.61   NaN%
```
...
![Backtest of a simple SMA crossover strategy. Signal quality depends heavily on market regime.](/courses/financial-statistics-1-foundations/../figures/06-1_ma-crossover-strategy-1.png)
...
![MACD consists of three components: the MACD line (fast EMA - slow EMA), the signal line (EMA of MACD), and the histogram (difference between them).](/courses/financial-statistics-1-foundations/../figures/06-1_macd-calculation-1.png)

``` r
print(p2)
```

![MACD consists of three components: the MACD line (fast EMA - slow EMA), the signal line (EMA of MACD), and the histogram (difference between them).](/courses/financial-statistics-1-foundations/../figures/06-1_macd-calculation-2.png)
...
![MACD divergence: when price makes new highs but MACD doesn't (or vice versa), it often signals a reversal.](/courses/financial-statistics-1-foundations/../figures/06-1_macd-divergence-1.png)
...
##          Strategy Ann. Return Ann. Vol Sharpe Max DD
##            <char>      <char>   <char> <char> <char>
## 1:     Buy & Hold       12.9%    17.4%   0.70   NaN%
## 2: MACD Crossover        6.9%    10.0%   0.67   NaN%
```
...
![ADX measures trend strength. Values above 25 indicate strong trends; values below 20 suggest ranging markets.](/courses/financial-statistics-1-foundations/../figures/06-1_adx-calculation-1.png)

``` r
print(p2)
```

![ADX measures trend strength. Values above 25 indicate strong trends; values below 20 suggest ranging markets.](/courses/financial-statistics-1-foundations/../figures/06-1_adx-calculation-2.png)
...
```
## === ADX-Filtered Strategy Comparison ===
```
...
##             <char>      <char> <char>         <char>
## 1:      Buy & Hold        NaN%    NaN           100%
## 2:    MA Crossover        NaN%    NaN           NaN%
## 3: ADX-Filtered MA        NaN%    NaN           NaN%
```
...
![Using ADX as a regime filter: only trade trend-following signals when ADX > 25.](/courses/financial-statistics-1-foundations/../figures/06-1_adx-regime-filter-1.png)
...
```
##   <U+2022> Use ADX > 25 to filter trend-following signals
```
...
```
##   <U+2022> Use ADX < 20 to apply mean-reversion strategies
```
...
```
##   <U+2022> Rising ADX = strengthening trend
```
...
```
##   <U+2022> Falling ADX = weakening trend
```
