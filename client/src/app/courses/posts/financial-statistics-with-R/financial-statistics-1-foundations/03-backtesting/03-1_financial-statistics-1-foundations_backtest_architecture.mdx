...
<Figure src="/courses/financial-statistics-1-foundations/pipeline-diagram-1.png" alt="The backtesting pipeline transforms data into performance metrics through five stages. Each stage must maintain point-in-time correctness.">
	The backtesting pipeline transforms data into performance metrics through five stages. Each stage must maintain point-in-time correctness.
</Figure>
...
<Figure src="/courses/financial-statistics-1-foundations/simple-backtest-example-1.png" alt="A complete backtest example: moving average crossover on S&amp;P 500. The strategy is simple but illustrates all pipeline components.">
	A complete backtest example: moving average crossover on S&amp;P 500. The strategy is simple but illustrates all pipeline components.
</Figure>
...
MA Crossover Backtest Results:
  ...
  Sharpe Ratio: -0.01
  Max Drawdown: 60.69%
  Annual Turnover: 10.0x
  Total Costs: 2090 bps
```
...
<Figure src="/courses/financial-statistics-1-foundations/signal-examples-1.png" alt="Three different signal types applied to S&amp;P 500. Each captures different market dynamics.">
	Three different signal types applied to S&amp;P 500. Each captures different market dynamics.
</Figure>
...
<Figure src="/courses/financial-statistics-1-foundations/signal-return-scatter-1.png" alt="Signal-return relationships. Effective signals show correlation between today&#39;s signal and tomorrow&#39;s return.">
	Signal-return relationships. Effective signals show correlation between today's signal and tomorrow's return.
</Figure>
...
<Figure src="/courses/financial-statistics-1-foundations/rolling-ic-1.png" alt="Rolling 1-year IC for momentum and mean reversion signals. Signal effectiveness varies through time.">
	Rolling 1-year IC for momentum and mean reversion signals. Signal effectiveness varies through time.
</Figure>
...
<Figure src="/courses/financial-statistics-1-foundations/position-mappings-1.png" alt="Different position mapping functions convert signals to positions. Each has different characteristics.">
	Different position mapping functions convert signals to positions. Each has different characteristics.
</Figure>
...
<Figure src="/courses/financial-statistics-1-foundations/turnover-comparison-1.png" alt="Turnover differs dramatically by mapping function. Threshold creates discrete jumps; sigmoid is smoother.">
	Turnover differs dramatically by mapping function. Threshold creates discrete jumps; sigmoid is smoother.
</Figure>

```

Turnover by Mapping Method:
      Method Annual Turnover Days with Trade
      <char>           <num>           <int>
1: Threshold        56.75118             596
2:    Linear        55.40014            4904
3:   Sigmoid        56.87693            4798
```
...
Raw vs Vol-Targeted Strategy:
  Raw - Sharpe: -0.37, Vol: 10.8%, MaxDD: 63.4%
  Vol-Targeted - Sharpe: -0.60, Vol: 9.3%, MaxDD: 73.9%
```
...
<Figure src="/courses/financial-statistics-1-foundations/vol-targeting-comparison-1.png" alt="Vol-targeting stabilises strategy volatility over time, reducing drawdowns during high-volatility periods.">
	Vol-targeting stabilises strategy volatility over time, reducing drawdowns during high-volatility periods.
</Figure>
