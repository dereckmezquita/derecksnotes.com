...
<Figure src="/courses/financial-statistics-1-foundations/order-book-simulation-1.png" alt="Simulated order book showing bid and ask queues. The spread is the gap between the best bid and best ask.">
	Simulated order book showing bid and ask queues. The spread is the gap between the best bid and best ask.
</Figure>
...
<Figure src="/courses/financial-statistics-1-foundations/spread-by-size-1.png" alt="Bid-ask spread as percentage of price across different market capitalisations. Smaller stocks have wider spreads.">
	Bid-ask spread as percentage of price across different market capitalisations. Smaller stocks have wider spreads.
</Figure>
...
```
Order book simulation summary:
  Mean spread: <U+00A3> 0 
  Mean spread (bps): NaN 
  Price range: <U+00A3> 0 to <U+00A3> 0 
```
...
<Figure src="/courses/financial-statistics-1-foundations/plot-ob-simulation-1.png" alt="Simulated order book dynamics showing mid price, bid, and ask over time.">
	Simulated order book dynamics showing mid price, bid, and ask over time.
</Figure>
...
Effective spread estimation:
  True spread (mean): NaN bps
  Roll estimate: NA (positive autocovariance)
```
...
SPY data structure:
  ...
  Date range: 2005-01-03 to 2026-01-16 
  Total observations: 5294 
Key: <date>
         date   open   high    low  close    volume adjusted      returns
       <Date>  <num>  <num>  <num>  <num>     <num>    <num>        <num>
1: 2024-01-02 472.16 473.67 470.49 472.65 123623700 461.2474 -0.005612126
2: 2024-01-03 470.43 471.19 468.17 468.79 103585900 457.4806 -0.008200112
3: 2024-01-04 468.30 470.96 467.05 467.28  84232200 456.0070 -0.003226324
4: 2024-01-05 467.49 470.44 466.43 467.92  86118900 456.6316  0.001368719
5: 2024-01-08 468.43 474.75 468.30 474.60  74879100 463.1505  0.014175097
6: 2024-01-09 471.87 474.93 471.35 473.88  65931400 462.4478 -0.001518299
   returns_simple
            <num>
1:   -0.005596407
2:   -0.008166583
...
<Figure src="/courses/financial-statistics-1-foundations/candlestick-chart-1.png" alt="Candlestick chart showing OHLC price data. Green candles indicate close &gt; open (up days), red indicate close &lt; open (down days).">
	Candlestick chart showing OHLC price data. Green candles indicate close &gt; open (up days), red indicate close &lt; open (down days).
</Figure>
...
<Figure src="/courses/financial-statistics-1-foundations/ohlc-info-loss-1.png" alt="Information loss from OHLCV aggregation. The same OHLC values can arise from very different intraday paths.">
	Information loss from OHLCV aggregation. The same OHLC values can arise from very different intraday paths.
</Figure>
...
```
SPY Volatility Estimates (Annualised):
         Estimator Volatility
            <char>     <char>
1:  Close-to-Close     19.09%
2:       Parkinson     15.33%
3:    Garman-Klass     15.54%
4: Rogers-Satchell     15.81%
```
...
<Figure src="/courses/financial-statistics-1-foundations/rolling-vol-comparison-1.png" alt="Rolling 20-day volatility using different estimators. Range-based estimators react more quickly to volatility changes.">
	Rolling 20-day volatility using different estimators. Range-based estimators react more quickly to volatility changes.
</Figure>
...
```
AAPL data loaded:
  Date range: 2000-01-03 to 2026-01-16 
  Has adjusted column: TRUE 
```
...
<Figure src="/courses/financial-statistics-1-foundations/aapl-split-viz-1.png" alt="Apple stock price showing the effect of adjustment for the 4-for-1 split in August 2020.">
	Apple stock price showing the effect of adjustment for the 4-for-1 split in August 2020.
</Figure>
...
<Figure src="/courses/financial-statistics-1-foundations/dividend-effect-1.png" alt="Effect of dividend adjustments on cumulative returns. Unadjusted prices understate total return for dividend-paying stocks.">
	Effect of dividend adjustments on cumulative returns. Unadjusted prices understate total return for dividend-paying stocks.
</Figure>
...
Return comparison over 2 years:
  Adjusted (true total return): -1 %
  Unadjusted (price only): -5.9 %
  Difference (missed dividends): 4.8 %
```
...
```
Split adjustment example:
          date close adjusted adj_factor
        <Date> <num>    <num>      <num>
 1: 2024-01-01   100       50        0.5
 2: 2024-01-02   102       51        0.5
 3: 2024-01-03   104       52        0.5
 4: 2024-01-04    52       52        1.0
 5: 2024-01-05    53       53        1.0
 6: 2024-01-06    54       54        1.0
 7: 2024-01-07    55       55        1.0
 8: 2024-01-08    56       56        1.0
 9: 2024-01-09    57       57        1.0
10: 2024-01-10    58       58        1.0
```
...
box::use(../modules/data[load_market, load_returns, list_market_symbols])
