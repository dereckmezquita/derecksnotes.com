...
```{r stop-loss-concept, fig.cap="Stop-loss truncates the return distribution. You trade the left tail for a spike of probability mass at the stop level."}
...
```{r stop-loss-impact, fig.cap="The impact of stop-loss on a trending strategy. Stops can help in mean-reverting markets but often hurt trend-followers."}
...
```{r stop-level-optimization, fig.cap="Stop-loss level vs Sharpe ratio. There's often an optimal stop level, but it's strategy-dependent."}
...
```{r leverage-blowup, fig.cap="Higher leverage means faster growth but also higher probability of catastrophic loss. At some point, more leverage decreases expected terminal wealth."}
...
```{r concentration-risk, fig.cap="Concentration risk: even with the same expected return, concentrated portfolios have much wider outcome distributions."}
...
```{r position-limits-implementation}
...
```{r drawdown-control-concept, fig.cap="Drawdown-based position reduction: as drawdown increases, position size decreases linearly until a maximum drawdown threshold where the position is fully exited."}
...
```{r cppi-implementation, fig.cap="CPPI (Constant Proportion Portfolio Insurance) maintains exposure proportional to cushion above a floor value."}
# CPPI implementation
...
```{r cppi-exposure-over-time, fig.cap="CPPI exposure varies dynamically. During crashes, exposure drops to protect the floor; during rallies, exposure increases."}
...
```{r dd-control-visualisation, fig.cap="Drawdown control in action: position size automatically reduces during losing periods, protecting capital."}
# Visualise
