...
```{r sharpe-distribution, fig.cap="Sampling distribution of Sharpe ratio. A true Sharpe of 0.5 can easily produce observed values from 0 to 1.0 over 3 years."}
...
```{r sharpe-confidence-intervals, fig.cap="95% confidence intervals for Sharpe ratio estimation. Even 10 years of data has substantial uncertainty."}
...
```{r required-track-record, fig.cap="Minimum track record length to achieve statistical significance at various Sharpe ratios."}
...
```{r regime-classification, fig.cap="Market regime classification based on volatility and trend indicators."}
...
```{r regime-strategy-analysis, fig.cap="Strategy performance varies dramatically by regime. Understanding these patterns is essential."}
...
```{r regime-analysis-implementation}
...
```{r regime-transition-analysis, fig.cap="Performance following regime transitions reveals adaptation patterns."}
...
```{r minimum-sharpe-surface, fig.cap="Minimum believable Sharpe as a function of track record and number of tests."}
...
```{r sharpe-reality-check, fig.cap="Decomposition of minimum Sharpe requirements by source of uncertainty."}
...
```{r final-decision-visual, fig.cap="Strategy evaluation dashboard summarising all key metrics and thresholds."}
