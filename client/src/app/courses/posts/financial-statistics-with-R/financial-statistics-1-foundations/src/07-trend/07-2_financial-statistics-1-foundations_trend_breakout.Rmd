...
```{r donchian-channels, fig.cap="Donchian Channels create a trading range from highest highs and lowest lows. Breakouts above/below these levels signal trend initiation."}
...
```{r turtle-entries-exits, fig.cap="Turtle Trading system: enter on 20-day breakouts, exit on 10-day breakouts in the opposite direction. The asymmetry helps cut losses quickly."}
...
```{r channel-asymmetry, fig.cap="Effect of entry/exit asymmetry on trade outcomes. Higher ratios (longer entries, shorter exits) create more favourable risk/reward profiles."}
...
```{r turtle-with-sizing, fig.cap="Complete Turtle system with ATR-based position sizing. Position size is inverse to volatility: smaller positions in volatile markets, larger in calm markets."}
...
```{r vol-compression, fig.cap="Volatility compression and expansion cycle. After periods of low ATR (shaded), price often makes large directional moves."}
...
```{r volatility-breakout-signals, fig.cap="Volatility breakout signals: enter when price moves more than k×ATR from previous close. The threshold k determines signal frequency and quality."}
...
```{r volatility-breakout-strategy, fig.cap="Volatility breakout strategy: enter on 2×ATR moves, hold for 5 days. The strategy captures explosive moves but suffers from false breakouts in choppy markets."}
...
```{r breakout-parameter-sensitivity, fig.cap="Breakout parameter sensitivity: different ATR multipliers and holding periods produce varying results. The optimal zone is relatively stable."}
...
```{r adx-filter-comparison, fig.cap="ADX filter for breakout systems. When ADX < 25 (ranging market), breakout signals are more likely to fail."}
# ADX calculation
...
```{r filtered-vs-unfiltered, fig.cap="Impact of ADX filter on Donchian breakout system. Filtering reduces whipsaws but may miss some trend starts."}
...
```{r multi-filter-system, fig.cap="Multi-filter trend system combining ADX and MA filters. Both conditions must be met for entry, reducing false signals significantly."}
# Multi-filter system
spy <- load_market("SPY")
spy <- filter_dates(spy, as.Date("2010-01-01"), as.Date("2023-12-31"))
spy[, returns := log(adjusted / shift(adjusted))]
