...
<Figure src="/courses/statistics-1-foundations/statistics_essence-1.png" alt="Statistics bridges the gap between sample observations and population truth">
	Statistics bridges the gap between sample observations and population truth
</Figure>
...
#>        group     n responders proportion  ci_lower  ci_upper
#>       <char> <int>      <int>      <num>     <num>     <num>
#> 1: Treatment    50         31       0.62 0.4815045 0.7413721
#> 2:   Control    50         21       0.42 0.2937500 0.5576656
...
ggplot2$ggplot(descriptive_summary, ggplot2$aes(x = group, y = proportion, fill = group)) +
    ...
    ggplot2$geom_errorbar(ggplot2$aes(ymin = ci_lower, ymax = ci_upper), width = 0.2, size = 1) +
...
<Figure src="/courses/statistics-1-foundations/descriptive_vs_inferential-1.png" alt="Descriptive statistics summarises data; inferential statistics generalises to populations">
	Descriptive statistics summarises data; inferential statistics generalises to populations
</Figure>
...
ggplot2$ggplot(sampling_dist, ggplot2$aes(x = sample_mean)) +
    ...
    ggplot2$geom_vline(xintercept = mu, colour = "red", linetype = "dashed", size = 1.2) +
    ggplot2$labs(
...
<Figure src="/courses/statistics-1-foundations/parameter_vs_statistic-1.png" alt="Statistics vary from sample to sample; parameters are fixed">
	Statistics vary from sample to sample; parameters are fixed
</Figure>
...
# Plot continuous
p1 <- ggplot2$ggplot(continuous_data, ggplot2$aes(x = value)) +
...
# Plot discrete
p2 <- ggplot2$ggplot(discrete_data, ggplot2$aes(x = value)) +
    ggplot2$geom_bar(fill = "#56B4E9", colour = "white", alpha = 0.8) +
    ggplot2$labs(
...
#> [1] "SRS sample of 50 from population of 1000:"
print(head(srs_sample, 20))
#>  [1] 835 423  29 368 594 777 768 969  21 861 979 868 652 337 977 649 150 978 633
#> [20] 926
```
...
stratified_sample <- function(data, strata_var, n_per_stratum) {
    ...
    return(data.table$rbindlist(samples))
}

# Create example population
set.seed(404)
pop_data <- data.table$data.table(
    id = 1:1000,
...
#> [1] "Stratified sample (25 per region):"
...
#> [1] "Systematic sample (every 20th, random start):"
print(head(sys_sample, 20))
#>  [1]  17  37  57  77  97 117 137 157 177 197 217 237 257 277 297 317 337 357 377
#> [20] 397
```
...
#> True population mean: 4.5 hours/week
cat("Respondent mean:", round(biased_mean, 2), "hours/week\n")
#> Respondent mean: 4.69 hours/week
cat("Bias:", round(biased_mean - true_mean, 2), "hours/week\n")
#> Bias: 0.19 hours/week
cat("Response rate:", round(100 * mean(pop$responds), 1), "%\n")
#> Response rate: 51 %

# Visualise
ggplot2$ggplot(pop, ggplot2$aes(x = exercise_hours, fill = factor(responds))) +
    ggplot2$geom_histogram(position = "identity", alpha = 0.6, bins = 40) +
    ggplot2$geom_vline(xintercept = true_mean, colour = "red", linetype = "dashed", size = 1) +
    ggplot2$geom_vline(xintercept = biased_mean, colour = "blue", linetype = "dotted", size = 1) +
    ggplot2$labs(
    ...
    ggplot2$scale_fill_manual(values = c("0" = "#999999", "1" = "#0072B2"),
                      labels = c("Non-respondent", "Respondent")) +
    ggplot2$theme_minimal()
```

<Figure src="/courses/statistics-1-foundations/sampling_bias_simulation-1.png" alt="Non-response can create substantial bias">
	Non-response can create substantial bias
</Figure>
...
demo_pop <- data.table$data.table(
    id = 1:500,
...
#> Stratified sample size: 48
cat("Cluster sample size:", nrow(clust), "\n")
#> Cluster sample size: 137
...
target_data <- data.table$data.table(
...
set.seed(909)
target_data_2d <- data.table$data.table(
    scenario = rep(c(
        "Low Bias, Low Variability",
        "Low Bias, High Variability",
        "High Bias, Low Variability",
...
ggplot2$ggplot(target_data_2d, ggplot2$aes(x = x, y = y)) +
    ggplot2$geom_point(alpha = 0.6, colour = "#0072B2") +
    ggplot2$geom_point(ggplot2$aes(x = 0, y = 0), colour = "red", size = 4, shape = 3, stroke = 2) +
...
<Figure src="/courses/statistics-1-foundations/bias_vs_variability-1.png" alt="Random error scatters around truth; systematic error shifts away from truth">
	Random error scatters around truth; systematic error shifts away from truth
</Figure>
