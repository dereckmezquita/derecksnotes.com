...
ggplot(plot_data, aes(x = value, fill = type)) +
    ...
    geom_vline(xintercept = true_mean, linetype = "dashed", colour = "red", size = 1) +
    geom_vline(xintercept = mean(sample_data), linetype = "dotted", colour = "blue", size = 1) +
    labs(
    ...
    scale_fill_manual(values = c("Population (N = 10,000)" = "#E69F00", "Sample (n = 100)" = "#56B4E9")) +
    theme_minimal() +
...
```
## Warning in geom_point(aes(x = 0, y = 0), colour = "red", size = 4, shape = 3, : All aesthetics have length 1, but the data has 200 rows.
