...
export default function NotFound() {
  ...
  useEffect(() => {
    ...
    function onMouseMoveDrag(e: MouseEvent) {
      if (dragStart && Vec2.from(e.clientX, e.clientY).distTo(dragStart) > 5) {
        isDragging = true;
      }
    }
    ...
    function draw() {
      ...
      while (accumulator >= PHYSICS_DT) {
        ...
        if (useHash) {
          ...
          qt = new QuadTree({ x: 0, y: 0, w, h }, 0, qtCapacityRef.current);
          ...
          if (useHash) {
            for (const p of particles) {
              ...
              for (const other of nearby) {
                if (other.id > p.id) Particle.attract(p, other, INTER_G);
              }
            }
          } else {
            for (const p of particles) {
      ...
      if (useHash) {
        ...
        renderQt = new QuadTree({ x: 0, y: 0, w, h }, 0, qtCapacityRef.current);
      ...
      if (dragStart && isDragging && mouse.active) {
        renderer.drawSlingshot(dragStart, mouse.pos, ballSizeRef.current);
      }
  ...
  return (
    <>
      ...
      >
        ...
        >
          {(['Size', 'Grid', 'Spawn', 'Cooldown', 'Overlap'] as const).map(
            (label) => {
              const configs = {
                Size: {
                  value: ballSize,
