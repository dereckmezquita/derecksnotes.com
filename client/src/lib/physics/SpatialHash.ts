...
export class SpatialHash {
    ...
    private cellFor(x: number, y: number): [number, number] {
        const col = Math.max(
            0,
            Math.min(this.cols - 1, (x / this.cellSize) | 0)
        );
        const row = Math.max(
            0,
            Math.min(this.rows - 1, (y / this.cellSize) | 0)
        );
        return [col, row];
    }
    ...
    queryRadius(
        ...
        const [minCol, minRow] = this.cellFor(
            center.x - radius,
            center.y - radius
        );
        const [maxCol, maxRow] = this.cellFor(
            center.x + radius,
            center.y + radius
        );
            ...
            for (let i = 0; i < cell.length; i++) {
                ...
                for (const [nc, nr] of neighbors) {
                    ...
                    for (const other of neighbor) {
                        if (absorbed.has(cell[i].id) || absorbed.has(other.id))
                            continue;
