...
export class QuadTree {
  ...
  private collideInternal(
    ...
    for (let i = 0; i < this.particles.length; i++) {
      ...
      for (let j = i + 1; j < this.particles.length; j++) {
        ...
        if (result) {
          onCollision(1);
          if (result === 'absorbed_a') absorbed.add(this.particles[i].id);
          if (result === 'absorbed_b') absorbed.add(this.particles[j].id);
        }
      }
  ...
  private checkAgainstChildren(
    ...
    const checkNode = (node: QuadTree) => {
      for (const other of node.particles) {
        if (p !== other && !absorbed.has(p.id) && !absorbed.has(other.id)) {
  ...
  private subdivide(): void {
    ...
    this.ne = new QuadTree({ x: x + hw, y, w: hw, h: hh }, d, this.capacity);
    this.sw = new QuadTree({ x, y: y + hh, w: hw, h: hh }, d, this.capacity);
