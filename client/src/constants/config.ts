...
export const APPLICATION_SUBDOMAIN: string =
    process.env.NEXT_PUBLIC_DEV_MODE === 'true' ? 'next.' : '';
export const APPLICATION_URL: string =
    APPLICATION_SUBDOMAIN + APPLICATION_DOMAIN;

// social images
export const APPLICATION_LOGO: string =
    APPLICATION_URL + '/site-images/512-logo.png';

// site info
export const APPLICATION_DESCRIPTION: string = 'Making sciencing easier.';
export const APPLICATION_AUTHOR: { first_name: string; last_name: string } = {
    first_name: 'Dereck',
    last_name: 'Mezquita'
};
...
export const API_PREFIX: string =
    process.env.NEXT_PUBLIC_DEV_MODE === 'true' ? '/api/v3' : '/api';
