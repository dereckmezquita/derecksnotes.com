...
function buildFlatHierarchy(
  ...
  for (const mdxFile of mdxFiles) {
    ...
    const partTitle = mdxData.frontmatter.title;
    const displayTitle = label ? `${label} ${index}: ${partTitle}` : partTitle;
...
export function loadSeriesMetadata(
  ...
  if (hasChapters) {
    ...
    hierarchy = buildFlatHierarchy(seriesDir, labels, frontmatter.published);
  }
...
export function getSectionContent(section: string): ContentCardMetadata[] {
  ...
  try {
    ...
    console.error(`Error scanning content for section "${section}":`, error);
    return [];
  }
...
export function getAllContentPaths(section: string): string[][] {
  ...
  try {
    ...
    console.error(`Error collecting paths for section "${section}":`, error);
    return [];
  }
  return paths;
}
