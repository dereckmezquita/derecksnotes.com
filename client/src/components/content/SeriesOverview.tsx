    ...
    if (node.isDirectory) {
      ...
      return (
        <ChapterBlock key={node.path}>
          ...
          {node.summary && <ChapterSummary>{node.summary}</ChapterSummary>}
              ...
              {publishedChildren.map((child) => (
                <PartItem key={child.path}>
                  {child.isDirectory ? (
                    ...
                    <PartLink href={`${seriesBaseUrl}/${child.path}`}>
                      {child.displayTitle}
                    </PartLink>
                  )}
                </PartItem>
    ...
    <PostContainer>
      ...
      <Article>
        ...
        <SeriesMeta>
          ...
          <span>{series.allParts.filter((p) => p.published).length} parts</span>
        </SeriesMeta>
        ...
        {series.comments && <Comments slug={slug} title={series.title} />}
      </Article>
    </PostContainer>
  );
}
