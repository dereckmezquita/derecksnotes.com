...
export const initGetDefinitions = (client: MongoClient) => {
    ...
    getDefinitions.post('/getDefinitions', (req, res) => {
        const { dictionary, pageSize, nextToken } = req.body;
        ...
        (async () => {
            ...
            // get the data which matches the request dictionary
            const { docs, nextID } = await page(collection, { dictionary: dictionary }, pageSize, new ObjectId(nextToken));
