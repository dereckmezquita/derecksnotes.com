...
export const ROOT_DIR_CLIENT_UPLOADS: string = path.join(ROOT_DIR_CLIENT, process.env.PROD_MODE ? 'out' : 'public', 'site-images', 'uploads', 'profile-photos');

// if in prod nginx picks up requests to /api/v3 and forwards to server
export const API_PREFIX: string = process.env.PROD_MODE ? '' : '/api/v3';
