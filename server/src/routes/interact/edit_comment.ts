...
edit_comment.patch('/edit_comment', isAuthenticated, isVerified, async (req: Request, res: Response) => {
    try {
        ...
        if (!comment) {
            return res.status(404).json({ message: "Comment not found or you do not own this comment." });
        }
        ...
        res.status(500).json({ message: "Unable to edit the comment. Please try again." });
    }
});
