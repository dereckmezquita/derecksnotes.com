    ...
    async (req: Request, res: Response) => {
        try {
            ...
            if (!geolocationIds || !Array.isArray(geolocationIds)) {
                return res
                    .status(400)
                    .json({ message: 'Geolocation Ids are required.' });
            }

            if (geolocationIds.length > 10) {
                return res.status(400).json({
                    message: 'Cannot delete more than 50 geolocations at once.'
                });
            }
            ...
            if (updateResult.modifiedCount === 0) {
                return res
                    .status(400)
                    .json({ message: 'Geolocations not found.' });
            }

            return res
                .status(200)
                .json({ message: 'Geolocations deleted successfully.' });
        } catch (error) {
            console.error(error);
            return res.status(500).json({
                message: 'An error occurred while deleting geolocations.'
            });
        }
    }
);
