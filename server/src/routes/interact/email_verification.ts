...
email_verification.get(
    ...
    async (req: Request, res: Response) => {
        try {
            ...
            if (
                user.email.tokenExpiry &&
                new Date(user.email.tokenExpiry).getTime() >
                    Date.now() - 1 * 60 * 60 * 1000
            ) {
                const time_remaining =
                    new Date(user.email.tokenExpiry).getTime() - Date.now();
                return res.status(400).json({
                    message:
                        'A verification request was recently sent. Please check your email or try again later.',
    ...
    `
            ...
            res.status(400).json({
                message: 'Error processing e-mail verification request: ',
                error: err
            });
        }
    }
...
email_verification.get(
    ...
    async (req: Request, res: Response) => {
        try {
            ...
            res.status(400).json({
                message: 'An error occurred trying to verify token.',
                error: err
            });
        }
    }
