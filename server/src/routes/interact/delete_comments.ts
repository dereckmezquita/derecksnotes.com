    ...
    async (req: Request, res: Response) => {
        try {
            ...
            if (!commentIds || !Array.isArray(commentIds)) {
                return res
                    .status(400)
                    .json({ message: 'Comment Ids are required.' });
            }

            if (commentIds.length > 50) {
                return res.status(400).json({
                    message: 'Cannot delete more than 50 comments at once.'
                });
            }
            ...
            const userIds = await Comment.find({
                _id: { $in: commentIds }
            }).distinct('userId');
            const commentsNotOwned = userIds.filter(
                (id) => id.toString() !== userId
            );

            if (commentsNotOwned.length) {
                return res.status(401).json({
                    message: `You do not own these comments: ${commentsNotOwned.join(', ')}`
                });
            }

            let deletedComments = await Comment.deleteManyOwnedByUser(
                commentIds,
                userId
            );
