...
import {
    DATETIME_YYYY_MM_DD_HHMMSS,
    ROOT_DIR_CLIENT_UPLOADS
        ...
        try {
            ...
            while (user.profilePhotos.length > 5) {
                ...
                if (removedPhoto) {
                    ...
                    fs.unlink(
                        path.join(ROOT_DIR_CLIENT_UPLOADS, removedPhoto),
                        (err) => {
                            if (err)
                                console.error(
                                    `Failed to delete photo: ${removedPhoto}`,
                                    err
                                );
                        }
                    );
            ...
            res.status(200).json({
                message: 'Image uploaded, processed, and saved',
                imageName: imageName
            });
        } catch (error) {
            console.error(
                'Error while processing image or updating user:',
                error
            );
